...
export class ResgateApiClient {
  async login(usuario: string, senha: string): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `API Resgate offline (${RESGATE_API_BASE}). Suba: cd 7-Resgate/src/CTe.Resgate.Api && dotnet run`
      );
    }
    if (!res.ok) {
  ...
  async enfileirarDownload(chaves: string[]): Promise<EnfileirarResult> {
    ...
    try {
      ...
      throw new Error(
        `Sem conexão com a API Resgate (${RESGATE_API_BASE}). Confira se :5070 está no ar.`
      );
    }
    if (!res.ok) {
