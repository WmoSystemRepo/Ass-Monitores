...
/// <summary>
/// Controle operacional do Arquivador em POC.
...
public sealed class WindowsServiceControllerAdapter : IWindowsServiceController
{
    ...
    public ServiceControlResult GetStatus(string serviceName)
    {
        ...
        {
            return new ServiceControlResult(true, "Running", "Arquivador CT-e ligado.");
        }
        ...
        if (_options.PreferLocalProcess)
        {
            if (TryResolveExePath(out var exePath, out var resolveError))
            {
                return new ServiceControlResult(
                    true,
                    "Stopped",
                    $"Arquivador CT-e desligado. Pronto para ligar ({exePath}).");
            }
            ...
            return new ServiceControlResult(
                ...
                "NotFound",
                resolveError
                ?? "Arquivador CT-e não disponível. Compile tools/Arquivador.DevHost (Debug).");
        }
        ...
        lock (_startLock)
        {
            ...
            if (_options.PreferLocalProcess)
            {
                return new ServiceControlResult(
                    ...
                    "Stopped",
                    killed > 0
                        ? $"Arquivador CT-e desligado. Detalhe técnico: host encerrado ({killed}); Executar=0."
                        : "Arquivador CT-e desligado. Detalhe técnico: nada em execução; Executar=0.");
            }
        ...
        try
        {
            ...
            if (process.HasExited)
            {
                return new ServiceControlResult(
                    false,
                    "Error",
                    $"Host saiu imediatamente (exit={process.ExitCode}). Verifique {psi.WorkingDirectory}\\Arquivador.DevHost.exe.config");
            }

            return new ServiceControlResult(
                true,
                "Running",
                $"Arquivador CT-e ligado (PID {process.Id}). Detalhe: Arquivador.DevHost — {exePath}");
        }
        catch (Exception ex)
        {
    ...
    private void EnsureDevConfigBesideExe(string exePath)
    {
        ...
        var src = Path.Combine(
            root,
            "dfend-cte-arquivador-windowsservices",
            "DFEND_CTe_Arquivador",
            "AppConfig",
            "Desenvolvimento",
            "DFEND_CTe_Arquivador.exe.config");
        if (!File.Exists(src))
        {
            return;
        }
    ...
    private bool TryResolveExePath(out string exePath, out string? error)
    {
        ...
        var relative = string.IsNullOrWhiteSpace(_options.ArquivadorExeRelativePath)
            ? @"tools\Arquivador.DevHost\bin\Debug\Arquivador.DevHost.exe"
            : _options.ArquivadorExeRelativePath;

        var root = FindArquivadorRoot();
        if (root is null)
        {
            error =
                "Não achei a pasta 2-Arquivador/ (precisa ter dfend-cte-arquivador-windowsservices + Arquivador.Api). " +
                "Se o Monitor.Api foi iniciado pelo Orquestrador (shadow), reinicie o Orquestrador após atualizar, " +
                "ou defina Monitor:ArquivadorRootPath.";
            return false;
        }
        ...
        if (!File.Exists(full))
        {
            var csproj = Path.Combine(root, @"tools\Arquivador.DevHost\Arquivador.DevHost.csproj");
            if (!DevHostMsBuild.TryBuild(csproj, full, out var buildError))
            {
                error =
                    buildError
                    ?? $"Host POC não encontrado: {full}. Compile tools\\Arquivador.DevHost (Debug) ou rode 0-Orquestrador\\tools\\build-devhosts.ps1.";
                return false;
            }
        }
    ...
    private string? FindArquivadorRoot()
    {
        if (!string.IsNullOrWhiteSpace(_options.ArquivadorRootPath)
            && Directory.Exists(_options.ArquivadorRootPath))
        {
            return Path.GetFullPath(_options.ArquivadorRootPath);
        }
                 ...
                 })
        {
            ...
            {
                var hasSvc = Directory.Exists(Path.Combine(dir.FullName, "dfend-cte-arquivador-windowsservices"));
                var hasApi = Directory.Exists(Path.Combine(dir.FullName, "Arquivador.Api"));
                if (hasSvc && hasApi)
                {
                    return dir.FullName;
                }
    ...
    private bool IsLocalHostRunning()
    {
        try
        {
            var hostName = string.IsNullOrWhiteSpace(_options.ArquivadorProcessName)
                ? "Arquivador.DevHost"
                : _options.ArquivadorProcessName;
