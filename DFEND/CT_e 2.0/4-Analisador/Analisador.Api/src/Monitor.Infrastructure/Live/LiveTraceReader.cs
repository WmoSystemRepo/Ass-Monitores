...
/// <summary>
/// Lê monitor-live.log gerado pelo Analisador.DevHost (Debug.WriteLine do Analisador).
...
public sealed class LiveTraceReader : ILiveTraceReader
{
    ...
    private string? FindAnalisadorRoot()
    {
        ...
        {
            var markerWs = Path.Combine(dir.FullName, "dfend-cte-analisador-windowsservices");
            var markerApi = Path.Combine(dir.FullName, "Analisador.Api");
            if (Directory.Exists(markerWs) && Directory.Exists(markerApi))
            {
                return dir.FullName;
            }

            // Também aceita pasta aninhada 4-Analisador/ como raiz
            var nested = Path.Combine(dir.FullName, "4-Analisador");
            if (Directory.Exists(Path.Combine(nested, "dfend-cte-analisador-windowsservices"))
                && Directory.Exists(Path.Combine(nested, "Analisador.Api")))
            {
                return nested;
            }
    ...
    private static LiveTraceLine? ParseLine(string raw)
    {
        ...
        // Linha crua do Analisador: DFEND_CTe_Analisador conexao iniciando...
