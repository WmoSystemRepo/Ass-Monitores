...
public sealed class MonitorOptions
{
    ...
    public int CodServicoArquivador { get; set; } = 3;
    public string WindowsServiceName { get; set; } = "DFEND_CTe_Arquivador";

    public string ServiceId { get; set; } = "dfend-cte-monitor-arquivador";
    public string DisplayName { get; set; } = "Monitor Arquivador CT-e";
    public string Domain { get; set; } = "arquivador";
    public string ApiVersion { get; set; } = "1.0";
    public string MonitoredService { get; set; } = "DFEND_CTe_Arquivador";
    ...
    /// <summary>Opcional. Se vazio, sobe pastas até achar raiz com dfend-cte-arquivador-windowsservices + Arquivador.Api.</summary>
    public string ArquivadorRootPath { get; set; } = string.Empty;
    /// <summary>
    /// Host POC (tools/Arquivador.DevHost) — NÃO é o Windows Service original.
    /// Relativo à raiz do clone Arquivador (pasta com windowsservices + Arquivador.Api).
    /// </summary>
    public string ArquivadorExeRelativePath { get; set; } =
        @"tools\Arquivador.DevHost\bin\Debug\Arquivador.DevHost.exe";
    /// <summary>Nome do processo do host POC (GetProcessesByName).</summary>
    public string ArquivadorProcessName { get; set; } = "Arquivador.DevHost";
...
public sealed class SnapshotAggregator : ISnapshotAggregator
{
    ...
    public async Task<MonitorSnapshot> BuildSnapshotAsync(CancellationToken ct)
    {
        ...
        try
        {
            var serviceResult = await _read.GetServiceAsync(_options.CodServicoArquivador, ct);
        ...
        CancellationToken ct)
    {
        ...
        var result = await _read.GetConfigsAsync(_options.CodServicoArquivador, ct);
        ...
        IReadOnlyList<LogEntry> logs)
    {
        ...
        for (var id = 1; id <= configured; id++)
        {
            ...
            list.Add(new ThreadView(
                ...
                false,
                last?.Mensagem ?? (id == 1 ? "Consome fila do Arquivador" : "Worker do pool"),
                last?.DtcLog,
                last?.CStat,
                last?.SeverityHint));
        }
...
public static class TableHealthBuilder
{
        ...
        AlertThresholdOptions thresholds)
    {
        ...
        // --- Serviço ---
        var nsu = service?.NumSequencialUnico ?? "—";
        var heartbeatAge = Age(service?.DtcExecucao);
        var servicoStatus = "Ok";
        var servicoHint = nsuAdvanced
            ? "Posição da busca avançou neste ciclo."
            : "Posição da busca estável neste ciclo.";
        if (executar == 0)
        {
            servicoStatus = "Atencao";
            servicoHint = "Arquivamento desligado — NSU não deve avançar.";
        }
        else if (heartbeatAge.HasValue && heartbeatAge.Value > staleSeconds)
        {
            servicoStatus = "Critico";
            servicoHint = "Última batida no banco antiga com arquivamento ligado.";
        }
        ...
        var servico = new TableHealthView(
            ...
            "Serviço (NSU)",
            servicoStatus,
            $"Posição da busca: {nsu}" + (nsuAdvanced ? " · avançou" : string.Empty),
            heartbeatAge,
            servicoQueryMs,
            servicoHint,
            "/tabelas/servico");
        ...
        var configPrimary = executar == 1 ? "Arquivamento ligado" : "Arquivamento desligado";
        ...
        {
            logStatus = "Critico";
            logHint = "Arquivamento ligado sem log novo (silêncio).";
        }
        else if (executar == 0)
        {
            logStatus = "Atencao";
            logHint = "Arquivamento desligado — poucos eventos esperados.";
        }
        ...
        {
            var status = "Ok";
            var hint = $"{label} estável.";
            if (depth >= thresholds.FilaAlta)
            {
                status = "Critico";
                hint = $"{label} alta.";
            }
            else if (key == "fila_entrada"
                     && brokerTrend.Count >= thresholds.FilaCrescendoSnapshots
                     && depth >= thresholds.FilaCrescendoMinDepth
                     && brokerTrend.TakeLast(thresholds.FilaCrescendoSnapshots).Zip(
                         brokerTrend.TakeLast(thresholds.FilaCrescendoSnapshots).Skip(1),
                         (a, b) => b >= a).All(x => x))
            {
                status = "Atencao";
                hint = "Fila de entrada crescendo nos últimos ciclos.";
            }

            return new TableHealthView(
                key,
                label,
                status,
                $"Na fila: {depth}",
        ...
        var filaEntrada = FilaView(
            "fila_entrada",
            "Fila entrada (Arquivador)",
            queues.BrokerDepth,
            queues.BrokerQueryMs,
            "/tabelas/fila_entrada");
        var filaSint = FilaView(
            "fila_sintetizador",
            "Fila Sintetizador",
            queues.SintetizadorDepth,
            0,
            "/tabelas/fila_sintetizador");
        var filaAnal = FilaView(
            "fila_analisador",
            "Fila Analisador",
            queues.AnalisadorDepth,
            0,
            "/tabelas/fila_analisador");
        var filaInt = FilaView(
            "fila_integrador",
            "Fila Integrador",
            queues.IntegradorDepth,
            0,
            "/tabelas/fila_integrador");

        return [servico, config, tmp, log, filaEntrada, filaSint, filaAnal, filaInt];
    }
}
...
public sealed class TableDetailService : ITableDetailService
{
    ...
    public async Task<TableDetailDto?> GetAsync(string key, int take, CancellationToken ct)
    {
        ...
        var banner = receptionOn
            ? null
            : "Arquivamento desligado — mostrando dados da última sessão (desde a última alteração de Executar).";

        return key switch
        {
            "servico" => await BuildServicoAsync(health, sessionStart, receptionOn, banner, take, ct),
            "configuracao" => await BuildConfigAsync(health, sessionStart, receptionOn, banner, ct),
            "temporaria" => await BuildTempAsync(health, sessionStart, receptionOn, banner, take, ct),
            ...
            "fila_sintetizador" => BuildFila(
                health, sessionStart, receptionOn, banner, snapshot.Queues.SintetizadorDepth, Array.Empty<long>()),
            "fila_analisador" => BuildFila(
                health, sessionStart, receptionOn, banner, snapshot.Queues.AnalisadorDepth, Array.Empty<long>()),
            "fila_integrador" => BuildFila(
                health, sessionStart, receptionOn, banner, snapshot.Queues.IntegradorDepth, Array.Empty<long>()),
            _ => null
        };
    }
    ...
    private async Task<TableDetailDto> BuildServicoAsync(
        ...
        CancellationToken ct)
    {
        var service = await _read.GetServiceAsync(_options.CodServicoArquivador, ct);
        var row = service.Row;
    ...
    private async Task<TableDetailDto> BuildConfigAsync(
        ...
        CancellationToken ct)
    {
        var cfg = await _read.GetProcessConfigsAsync(_options.CodServicoArquivador, ct);
        return new TableDetailDto(
            "configuracao",
            health.Label,
...
public sealed class ServiceControlService
{
    ...
    public async Task<ServiceControlResult> StartAsync(CancellationToken ct)
    {
        ...
        {
            await _write.SetExecutarAsync(_options.CodServicoArquivador, 1, ct);
    ...
    {
        var commandId = Guid.NewGuid().ToString("N");
        await _write.SetExecutarAsync(_options.CodServicoArquivador, 0, ct);
