...
public sealed class MonitorOptions
{
    ...
    public int CodServicoSintetizador { get; set; } = 8;
    public string WindowsServiceName { get; set; } = "DFEND_CTe_Sintetizador";

    public string ServiceId { get; set; } = "dfend-cte-monitor-sintetizador";
    public string DisplayName { get; set; } = "Monitor Sintetizador CT-e";
    public string Domain { get; set; } = "sintetizador";
    public string ApiVersion { get; set; } = "1.0";
    public string MonitoredService { get; set; } = "DFEND_CTe_Sintetizador";
    ...
    /// <summary>Opcional. Se vazio, sobe pastas até achar raiz com dfend-cte-sintetizador-windowsservices + Sintetizador.Api.</summary>
    public string SintetizadorRootPath { get; set; } = string.Empty;
    /// <summary>
    /// Host POC (tools/Sintetizador.DevHost) — NÃO é o Windows Service original.
    /// Relativo à raiz do clone Sintetizador (pasta com windowsservices + Sintetizador.Api).
    /// </summary>
    public string SintetizadorExeRelativePath { get; set; } =
    ...
    public string SintetizadorProcessName { get; set; } = "Sintetizador.DevHost";
...
public sealed class SnapshotAggregator : ISnapshotAggregator
{
    ...
    public async Task<MonitorSnapshot> BuildSnapshotAsync(CancellationToken ct)
    {
        ...
        try
        {
            var serviceResult = await _read.GetServiceAsync(_options.CodServicoSintetizador, ct);
        ...
        CancellationToken ct)
    {
        ...
        var result = await _read.GetConfigsAsync(_options.CodServicoSintetizador, ct);
        ...
        IReadOnlyList<LogEntry> logs)
    {
        ...
        for (var id = 1; id <= configured; id++)
        {
            ...
            list.Add(new ThreadView(
                ...
                false,
                last?.Mensagem ?? (id == 1 ? "Consome fila do Sintetizador" : "Worker do pool"),
                last?.DtcLog,
                last?.CStat,
                last?.SeverityHint));
        }
...
public static class TableHealthBuilder
{
        ...
        AlertThresholdOptions thresholds)
    {
        ...
        else if (heartbeatAge.HasValue && heartbeatAge.Value > staleSeconds)
        {
            ...
            servicoHint = "Última batida no banco antiga com síntese ligada.";
        }
        ...
        var servico = new TableHealthView(
            ...
            "Serviço",
            servicoStatus,
            $"Serviço sintetizador" + (nsuAdvanced ? " · ativo" : string.Empty),
            heartbeatAge,
            servicoQueryMs,
            servicoHint,
            "/tabelas/servico");
        ...
        {
            filaStatus = "Critico";
            filaHint = "Fila Sintetizador alta.";
        }
...
public sealed class TableDetailService : ITableDetailService
{
    ...
    private async Task<TableDetailDto> BuildServicoAsync(
        ...
        CancellationToken ct)
    {
        var service = await _read.GetServiceAsync(_options.CodServicoSintetizador, ct);
        var row = service.Row;
    ...
    private async Task<TableDetailDto> BuildConfigAsync(
        ...
        CancellationToken ct)
    {
        var cfg = await _read.GetProcessConfigsAsync(_options.CodServicoSintetizador, ct);
        return new TableDetailDto(
            "configuracao",
            health.Label,
...
public sealed class ServiceControlService
{
    ...
    public async Task<ServiceControlResult> StartAsync(CancellationToken ct)
    {
        ...
        {
            await _write.SetExecutarAsync(_options.CodServicoSintetizador, 1, ct);
        ...
        return new ServiceControlResult(
            ...
            string.IsNullOrWhiteSpace(after.Message)
                ? "Sintetizador CT-e: start aceito."
                : after.Message,
            commandId);
    }
    ...
    {
        var commandId = Guid.NewGuid().ToString("N");
        await _write.SetExecutarAsync(_options.CodServicoSintetizador, 0, ct);
