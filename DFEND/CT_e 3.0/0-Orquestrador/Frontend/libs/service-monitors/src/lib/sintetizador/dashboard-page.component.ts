...
import {
  SintetizadorAnatomyFlowComponent,
  type AnatomyStage,
  type FlyingPacket,
} from './anatomy-flow.component';

@Component({
  selector: 'lib-sintetizador-dashboard-page',
  standalone: true,
  imports: [DatePipe, RouterLink, SintetizadorAnatomyFlowComponent, TableHealthCardsComponent],
    ...
    <section class="dashboard-fit flex h-[calc(100vh-3rem)] max-h-[calc(100vh-3rem)] flex-col gap-2 overflow-hidden">
        ...
        <div class="min-w-0 flex flex-wrap items-center gap-2.5">
          <div class="min-w-0">
            ...
            <p class="text-[11px] text-zinc-400">
              Está sintetizando documentos agora?
            </p>
          </div>
      ...
      @if (isLimitedTelemetry()) {
        ...
        >
          ...
          @if (processUp()) {
            <span class="font-medium"> Processo no ar; síntese sem telemetria completa.</span>
          }
          <button
            type="button"
        ...
        >
          <span>
            @if (isLimitedTelemetry()) {
              ...
              Síntese pausada (Executar≠1). Ative para voltar a sintetizar CT-e.
            }
          </span>
          <button
            type="button"
          ...
          <div class="receptor-empty flex h-full flex-col items-center justify-center gap-4 rounded-xl border border-zinc-600/60 bg-zinc-950/40 px-6 text-center">
            <div class="max-w-md space-y-2">
              <p class="text-[10px] font-semibold uppercase tracking-[0.16em] text-violet-300/90">
                Pipeline do Sintetizador
              </p>
              <h2 class="text-lg font-semibold text-zinc-50">Sintetizador parado</h2>
              <p class="text-sm text-zinc-400">
                Ligue para sintetizar CT-e da fila e acompanhar o fluxo:
            ...
            <div class="flex flex-wrap items-center justify-center gap-2">
              ...
              <a
                routerLink="/monitores/sintetizador/mais-informacoes"
                class="rounded border border-zinc-600 px-3 py-2 text-xs font-medium text-violet-300 hover:bg-zinc-800"
              >
                Mais informações →
              </a>
            </div>
...
export class SintetizadorDashboardPageComponent {
  ...
  /** Status único legível em 2s — substitui Processo/Síntese fragmentados. */
  readonly heroStatus = computed(() => {
    ...
    if (this.isRunning()) {
      ...
      return {
        ...
        hint: 'Sintetizador ligado — aguardando próximo ciclo de síntese',
      };
    }
    if (this.processUp()) {
      return {
    ...
    return {
      ...
      label: 'Parado',
      detail: null as string | null,
      hint: 'Sintetizador desligado — não processa novos CT-e',
    };
  });
  ...
  readonly flowCaption = computed(() => {
    ...
    if (!this.isRunning()) {
      ...
      if (this.processUp()) {
        return 'Processo no ar, mas a síntese está pausada (Executar≠1). Ative o trabalho para ver o pipeline.';
      }
    ...
    if (!stage) {
      return 'Sintetizador ligado — aguardando próximo ciclo de síntese.';
    }
    ...
    switch (stage) {
      case 'fila':
        return nsuBit
          ? `Fila · ${nsuBit}`
          : act?.detail ?? 'Retirando NSU da fila do sintetizador…';
      case 'temp':
        return nsuBit
          ? `Temporária · ${nsuBit}`
  ...
  constructor() {
    ...
    // Logs reais do Sintetizador disparam a journey (Constante.Msg*).
    effect(() => {
      if (!this.isRunning()) return;
