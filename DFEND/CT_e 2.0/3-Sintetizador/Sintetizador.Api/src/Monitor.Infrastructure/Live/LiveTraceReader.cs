...
/// <summary>
/// Lê monitor-live.log gerado pelo Sintetizador.DevHost (Debug.WriteLine do Sintetizador).
...
public sealed class LiveTraceReader : ILiveTraceReader
{
    ...
    private string? FindSintetizadorRoot()
    {
        ...
        {
            var markerWs = Path.Combine(dir.FullName, "dfend-cte-sintetizador-windowsservices");
            var markerApi = Path.Combine(dir.FullName, "Sintetizador.Api");
            if (Directory.Exists(markerWs) && Directory.Exists(markerApi))
            {
                return dir.FullName;
            }

            // Também aceita pasta aninhada 2-Sintetizador/ como raiz
            var nested = Path.Combine(dir.FullName, "2-Sintetizador");
            if (Directory.Exists(Path.Combine(nested, "dfend-cte-sintetizador-windowsservices"))
                && Directory.Exists(Path.Combine(nested, "Sintetizador.Api")))
            {
                return nested;
            }
    ...
    private static LiveTraceLine? ParseLine(string raw)
    {
        ...
        // Linha crua do Sintetizador: DFEND_CTe_Sintetizador conexao iniciando...
