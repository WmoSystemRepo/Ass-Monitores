...
@Component({
  selector: 'lib-sintetizador-details-page',
  standalone: true,
  ...
  template: `
    ...
    >
      <header class="flex shrink-0 flex-wrap items-center justify-between gap-2">
        <div>
          <p class="text-[10px] uppercase tracking-[0.2em] text-zinc-500">Diagnóstico</p>
          <h1 class="text-xl font-semibold text-zinc-50">Mais informações</h1>
        </div>
        <div class="flex flex-wrap items-center gap-3">
          <a routerLink="/monitores/sintetizador/logs" class="text-xs text-violet-400 hover:underline">Histórico →</a>
          <a routerLink="/monitores/sintetizador" class="text-xs text-violet-400 hover:underline">← Voltar ao Monitor</a>
        </div>
      </header>

      <div class="grid min-h-0 flex-1 grid-cols-1 gap-2 lg:grid-cols-2">
        <section
          class="flex min-h-0 flex-col rounded border border-zinc-700 bg-zinc-950/40 p-3"
        >
          <div class="mb-1.5 shrink-0">
            <h2 class="text-sm font-medium text-zinc-200">Passos do Sintetizador · online</h2>
            <p class="text-[10px] text-zinc-500">Debug do DevHost + SQL — alimentam o AGORA do Monitor</p>
          </div>
          <div class="min-h-0 flex-1 space-y-1 overflow-hidden">
            @for (row of flowFeed(); track row.key) {
              <div class="flex gap-2 text-[11px] leading-snug text-zinc-400">
                @if (row.at) {
                  <span class="shrink-0 font-mono text-zinc-600">{{ row.at | date: 'HH:mm:ss' }}</span>
                }
                <span
                  ...
                  [class.bg-zinc-800]="row.kind === 'sql'"
                  [class.text-zinc-300]="row.kind === 'sql'"
                  >{{ row.kind }}</span
                >
                <span class="min-w-0 truncate text-zinc-200">{{ row.text }}</span>
              </div>
            } @empty {
              <p class="text-xs text-zinc-500">
                Sem passos Debug/SQL. Ligue o Sintetizador pelo Monitor.
              </p>
            }
          </div>
        </section>
        ...
        >
          <div class="mb-1.5 shrink-0">
            <h2 class="text-sm font-medium text-zinc-200">Últimos eventos SQL do Sintetizador</h2>
            <p class="text-[10px] text-zinc-500">Classificados no banco (sucesso, erro, aviso)</p>
          </div>
          <div class="min-h-0 flex-1 space-y-1 overflow-hidden">
            @for (l of recentActivity(); track l.seqLog; let i = $index) {
              ...
              <p class="text-xs text-zinc-500">Ainda sem eventos SQL.</p>
            }
          </div>
        </section>
        ...
        >
          <div class="mb-1.5 flex shrink-0 flex-wrap items-center justify-between gap-2">
            <div>
              <h2 class="text-sm font-medium text-zinc-200">Sintetizador · parâmetros e lotes</h2>
              <p class="text-[10px] text-zinc-500">Configuração ativa e CT-e recentes na temporária</p>
            </div>
            <div class="flex flex-wrap gap-2">
              <a routerLink="/monitores/sintetizador/config" class="text-[10px] text-violet-400 hover:underline"
                >Config →</a
              >
            </div>
          </div>
          <div class="flex shrink-0 flex-wrap gap-x-3 gap-y-1 text-[11px]">
            <span class="text-zinc-500"
              >NSU
              <span class="font-mono text-zinc-200">{{ store.global()?.mainNsu || '—' }}</span></span
            >
            <span class="text-zinc-500"
              >Intervalo <span class="text-zinc-200">{{ intervaloLabel() }}</span></span
            >
            <span class="text-zinc-500"
              >Pacote completo
            ...
            >
            <span class="text-zinc-500" title="Threads"
              >Linhas
              <span class="text-zinc-200">{{ store.global()?.configuredThreads }}</span></span
            >
          </div>
          <div class="mt-2 min-h-0 flex-1 space-y-1 overflow-hidden border-t border-zinc-800 pt-2">
            @for (lote of recentLotes(); track lote.nsu + (lote.dtcAtualizacao ?? '')) {
              ...
              <p class="text-[11px] text-zinc-500">Nenhum lote recente na temporária.</p>
            }
          </div>
        </section>
        ...
        >
          <div class="mb-1.5 shrink-0">
            <h2 class="text-sm font-medium text-zinc-200">Avisos do Monitor</h2>
            <p class="text-[10px] text-zinc-500">Alertas de saúde, fila, NSU e regras RN</p>
          </div>
          <div class="min-h-0 flex-1 space-y-1 overflow-hidden">
            @for (a of topAlerts(); track a.code + a.message) {
              <div class="rounded border px-2 py-1 text-[11px]" [ngClass]="alertBorder(a.severity)">
                ...
                >
                <p class="truncate text-zinc-200">{{ a.message }}</p>
              </div>
            } @empty {
              <p class="text-xs text-zinc-500">Nenhum aviso do Monitor no momento.</p>
            }
          </div>
        </section>
      </div>
...
export class SintetizadorDetailsPageComponent {
