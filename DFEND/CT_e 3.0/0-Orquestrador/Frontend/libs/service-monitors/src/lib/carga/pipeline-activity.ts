 ...
 * para o ciclo da Carga: fila → chave → WS → persistir → limpar/órfão.
 */
export function describeLogActivity(log: LogLike): PipelineActivity | null {
    ...
    return {
      ...
      detail: [
        ...
        ivSec != null && !Number.isNaN(ivSec)
          ? `Download pontual a cada ${ivSec}s`
          : null,
      ]
        .filter(Boolean)
  ...
  if (msg.includes('serviço iniciado com sucesso')) {
    ...
    return {
      stage: 'bootstrap',
      headline: 'Carga ligado',
      detail: machine
        ? `Serviço iniciou na máquina ${machine}`
        : 'Serviço do Carga acabou de iniciar',
      threadId,
      at: log.dtcLog,
      source,
    };
  ...
  if (msg.includes('processo não iniciado')) {
    return {
      ...
      detail: 'Executar=0 — não sintetiza até ligar o Carga',
      threadId,
      at: log.dtcLog,
      source,
    };
  ...
  if (
    ...
    return {
      ...
      detail: nsu
        ? `${threadName} retirou chave ${nsu} da fila_alvo_cte_integrador`
  ...
  if (msg.includes('lote obtido no banco') || msg.includes('dado obtido no banco')) {
    return {
      ...
      detail: nsu
        ? `${threadName} obteve chave NSU ${nsu} em tmp_integracao`
        : `${threadName} leu a chave na temporária Integrador`,
      threadId,
      nsu,
      at: log.dtcLog,
      source,
    ...
    return {
      ...
      detail: nsu
        ...
        : isError
          ...
          : `${threadName} concluiu download (excluiu temp)`,
      threadId,
      nsu,
      at: log.dtcLog,
      source,
