...
import { monitorConnectionLabel, integradorStatusLabel } from '@integrador/shared-utils';
      ...
      <div class="flex min-h-screen">
        <aside class="w-60 shrink-0 border-r border-zinc-800 bg-zinc-900/80 p-4">
          ...
          <div class="mt-8 text-xs text-zinc-500">
            ...
            <p class="mt-2 text-sm text-zinc-300">Integrador · {{ statusLabel() }}</p>
          </div>
        </aside>
...
export class AppComponent {
  ...
  readonly statusLabel = computed(() =>
    integradorStatusLabel(this.store.service()?.scmStatus, this.store.service()?.executar)
  );
  ...
  readonly links = [
    ...
    { path: '/tabelas', label: 'Tabelas', hint: 'Banco em tempo real' },
    {
      path: '/config',
      label: 'Configurações',
      hint: 'Somente leitura · origem Integrador',
    },
  ];
}
