...
/// <summary>
/// Controle operacional do Carga em POC.
...
public sealed class WindowsServiceControllerAdapter : IWindowsServiceController
{
    ...
    public ServiceControlResult GetStatus(string serviceName)
    {
        ...
        {
            return new ServiceControlResult(true, "Running", "Carga CT-e ligado.");
        }
        ...
        if (_options.PreferLocalProcess)
        {
            if (TryResolveExePath(out var exePath, out var resolveError))
            {
                return new ServiceControlResult(
                    true,
                    "Stopped",
                    $"Carga CT-e desligado. Pronto para ligar ({exePath}).");
            }
            ...
            return new ServiceControlResult(
                ...
                "NotFound",
                resolveError
                ?? "Carga CT-e não disponível. Compile tools/Carga.DevHost (Debug).");
        }
        ...
        lock (_startLock)
        {
            ...
            if (_options.PreferLocalProcess)
            {
                return new ServiceControlResult(
                    ...
                    "Stopped",
                    killed > 0
                        ? $"Carga CT-e desligado. Detalhe técnico: host encerrado ({killed}); Executar=0."
                        : "Carga CT-e desligado. Detalhe técnico: nada em execução; Executar=0.");
            }
        ...
        try
        {
            ...
            if (process.HasExited)
            {
                return new ServiceControlResult(
                    false,
                    "Error",
                    $"Host saiu imediatamente (exit={process.ExitCode}). Verifique {psi.WorkingDirectory}\\Carga.DevHost.exe.config");
            }

            return new ServiceControlResult(
                true,
                "Running",
                $"Carga CT-e ligado (PID {process.Id}). Detalhe: Carga.DevHost — {exePath}");
        }
        catch (Exception ex)
        {
    ...
    private void EnsureDevConfigBesideExe(string exePath)
    {
        ...
        var src = Path.Combine(
            root,
            "dfend-cte-carga-windowsservices",
            "DFEND_CTe_Carga",
            "AppConfig",
            "Desenvolvimento",
            "DFEND_CTe_Carga.exe.config");
        if (!File.Exists(src))
        {
            return;
        }
    ...
    private bool TryResolveExePath(out string exePath, out string? error)
    {
        ...
        var relative = string.IsNullOrWhiteSpace(_options.CargaExeRelativePath)
            ? @"tools\Carga.DevHost\bin\Debug\Carga.DevHost.exe"
            : _options.CargaExeRelativePath;

        var root = FindCargaRoot();
        if (root is null)
        {
            error =
                "Não achei a pasta 6-Carga/ (precisa ter dfend-cte-carga-windowsservices + Carga.Api). " +
                "Se o Monitor.Api foi iniciado pelo Orquestrador (shadow), reinicie o Orquestrador após atualizar, " +
                "ou defina Monitor:CargaRootPath.";
            return false;
        }
        ...
        if (!File.Exists(full))
        {
            var csproj = Path.Combine(root, @"tools\Carga.DevHost\Carga.DevHost.csproj");
            if (!DevHostMsBuild.TryBuild(csproj, full, out var buildError))
            {
                error =
                    buildError
                    ?? $"Host POC não encontrado: {full}. Compile tools\\Carga.DevHost (Debug) ou rode 0-Orquestrador\\tools\\build-devhosts.ps1.";
                return false;
            }
        }
    ...
    private string? FindCargaRoot()
    {
        if (!string.IsNullOrWhiteSpace(_options.CargaRootPath)
            && Directory.Exists(_options.CargaRootPath))
        {
            return Path.GetFullPath(_options.CargaRootPath);
        }
                 ...
                 })
        {
            ...
            {
                var hasSvc = Directory.Exists(Path.Combine(dir.FullName, "dfend-cte-carga-windowsservices"));
                var hasApi = Directory.Exists(Path.Combine(dir.FullName, "Carga.Api"));
                if (hasSvc && hasApi)
                {
                    return dir.FullName;
                }
    ...
    private bool IsLocalHostRunning()
    {
        try
        {
            var hostName = string.IsNullOrWhiteSpace(_options.CargaProcessName)
                ? "Carga.DevHost"
                : _options.CargaProcessName;
