...
import { monitorConnectionLabel, analisadorStatusLabel } from '@analisador/shared-utils';
        ...
        <aside class="w-60 shrink-0 border-r border-zinc-800 bg-zinc-900/80 p-4">
          ...
          <nav class="mt-6 flex flex-col gap-1 text-sm">
            @for (l of links; track l.path) {
              <a
                [routerLink]="l.path"
                routerLinkActive="bg-zinc-800 text-violet-300"
          ...
          <div class="mt-8 text-xs text-zinc-500">
            ...
            <p class="mt-2 text-sm text-zinc-300">Analisador · {{ statusLabel() }}</p>
          </div>
        </aside>
...
export class AppComponent {
  ...
  readonly statusLabel = computed(() =>
    analisadorStatusLabel(this.store.service()?.scmStatus, this.store.service()?.executar)
  );
  ...
  readonly links = [
    ...
    { path: '/tabelas', label: 'Tabelas', hint: 'Banco em tempo real' },
    {
      path: '/config',
      label: 'Configurações',
      hint: 'Somente leitura · origem Analisador',
    },
  ];
}
