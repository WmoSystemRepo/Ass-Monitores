...
import { LogEntry } from '@analisador/shared-data';
import {
  LogKind,
  classifyLogKind,
  logKindLabel,
  ...
  template: `
    <section class="flex h-[calc(100vh-8rem)] flex-col gap-4">
      <header class="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p class="text-sm text-zinc-400">
            Linha do tempo completa do que o Analisador fez no banco.
            @if (store.live()) {
      ...
      <div class="relative min-h-0 flex-1 overflow-auto rounded border border-zinc-700 bg-zinc-950/80 p-4">
        <div class="absolute bottom-0 left-[1.65rem] top-4 w-px bg-gradient-to-b from-violet-500/40 via-zinc-700 to-transparent"></div>
        <div class="space-y-0">
          @for (l of visibleNewestFirst(); track l.seqLog; let i = $index) {
            ...
            >
              ...
              <div class="min-w-0 flex-1 rounded-lg border border-zinc-800 bg-zinc-900/50 px-3 py-2">
                <div class="flex flex-wrap items-center gap-2 text-[11px]">
                  ...
                  @if (l.threadId) {
                    <span class="text-violet-400/90">Linha {{ l.threadId }}</span>
                  }
                  @if (l.cStat) {
            ...
            <p class="p-6 text-center text-zinc-500">
              Nenhum evento neste filtro. Tente “Todos” ou ligue o Analisador.
            </p>
          }
        </div>
      </div>
