...
namespace DFe
{
    class Log
    {
        ...
        public Log(Facilitador clsFacilPar, string strConexaoPar, string strClassePar, string strNomeServicoPar, short intCodServicoPar, short intThreadPar, short intLogEventoPar, short intLogBancoPar, short intLogCompletoPar)
        {
        ...
        private string MontarMensagemLog(string strMetodo, string strMensagem, string strErroCompleto, EventLogEntryType objTipo)
        {
            ...
            // Adicionando o nome da aplicacao
            strLog = (strLog + Environment.NewLine + "Aplicação: " + strNomeServico + "; ");

            // Adicionando a versao da aplicacao
            strLog = (strLog + Environment.NewLine + "Versão: " + Assembly.GetEntryAssembly().GetName().Version.ToString().Remove(5) + "; ");
        ...
        {
            // Classes e variaveis utilizadas
            BdCTeSintetico clsBDExe = new BdCTeSintetico(clsFacil, strConexao);
