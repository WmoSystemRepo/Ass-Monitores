...
import { TableHealthCardsComponent } from '@arquivador/monitor-tables';
import {
...
@Component({
  ...
  standalone: true,
  imports: [DatePipe, AnatomyFlowComponent, TableHealthCardsComponent],
  ...
  template: `
    <section class="dashboard-fit flex h-[calc(100vh-3rem)] max-h-[calc(100vh-3rem)] flex-col gap-2 overflow-hidden">
      ...
      @if (actionBanner(); as banner) {
        <div class="shrink-0 rounded border border-amber-500/30 bg-amber-950/30 px-3 py-1 text-xs text-amber-100">
          <span class="font-medium">{{ banner.title }}</span>
          @if (banner.detail) {
            <span class="ml-1 font-mono text-[10px] text-amber-300/80">{{ banner.detail }}</span>
          }
        </div>
      }
      ...
      >
        <span class="inline-flex items-baseline gap-1.5">
          <span class="text-zinc-500">Arquivador</span>
...
export class DashboardPageComponent {
  ...
  readonly flowCaption = computed(() => {
    ...
    if (!stage) {
      return 'Arquivador ligado — aguardando próximo ciclo de arquivamento.';
    }
    ...
    switch (stage) {
      case 'fila':
        return nsuBit
          ? `Fila Arquivador · ${nsuBit}`
          : act?.detail ?? 'Retirando NSU da fila de entrada…';
      case 'temp':
        return nsuBit
          ? `Temp Recepção · ${nsuBit}`
          : act?.detail ?? 'Obtendo dado na temporária de recepção…';
      case 'sintetizador':
        return nsuBit
          ? `Sintetizador · ${nsuBit}`
          : act?.detail ?? 'Enviando para a fila do sintetizador…';
      case 'analisador':
        return nsuBit
          ? `Analisador · ${nsuBit}`
          : act?.detail ?? 'Analisando documento do lote…';
      case 'integrador':
        return nsuBit
          ? `Integrador · ${nsuBit}`
          : act?.detail ?? 'Integrando e excluindo temp…';
    }
  });

  /** Contagem regressiva até o próximo ciclo de arquivamento. */
  readonly cycleCountdown = computed(() => {
    ...
    if (epoch == null) {
      return {
        ...
        hint: `Próximo ciclo de arquivamento · a cada ${intervalo}s`,
      };
    }
    ...
    return {
      ...
      secondsLeft: left,
      hint: `Próximo ciclo de arquivamento em ${left}s (a cada ${intervalo}s)`,
    };
  });

  /**
   * Cronômetro de movimentação: sobe desde o último lote até novo arquivamento.
  ...
  readonly fileWaitChrono = computed(() => {
    ...
    // Filas caindo = arquivamento em andamento (não “sem arquivamento”).
    ...
    return {
      ...
      caption: 'sem arquivamento',
      display: this.formatElapsedClock(elapsed),
      hint: hasLote
        ? `Há ${elapsed}s sem arquivamento · último lote às ${new Date(loteAt).toLocaleTimeString('pt-BR')}`
  ...
  constructor() {
    ...
    effect(() => {
      ...
      const sint = queues?.sintetizadorDepth ?? 0;
      const anal = queues?.analisadorDepth ?? 0;
      const integ = queues?.integradorDepth ?? 0;
      ...
      const sintUp = queuesOk && this.lastSint >= 0 && sint > this.lastSint;
      const analUp = queuesOk && this.lastAnal >= 0 && anal > this.lastAnal;
      const integUp = queuesOk && this.lastInteg >= 0 && integ > this.lastInteg;
      ...
      } else if (!journeyBusy && sintUp) {
        this.pulseStage('sintetizador', 2200);
        this.spawnPackets(String(top?.nsu ?? 'sint'), 2, 2, 'cte');
      } else if (!journeyBusy && analUp) {
        this.pulseStage('analisador', 2200);
        this.spawnPackets(String(top?.nsu ?? 'anal'), 2, 3, 'cte');
      } else if (!journeyBusy && integUp) {
        this.pulseStage('integrador', 2200);
        this.spawnPackets(String(top?.nsu ?? 'integ'), 2, 4, 'envelope');
      }
    ...
    // Logs reais do Arquivador disparam a journey (Constante.Msg*).
    effect(() => {
      if (!this.isRunning()) return;
      ...
      if (msg.includes('chave retirada da fila') || msg.includes('lote obtido no banco')) {
        ...
        this.pulseStage('integrador', 2500);
  ...
  private playLoteJourney(nsu: number | null, qtd: number): void {
    ...
    const sequence: { stage: AnatomyStage; lane: number; kind: 'cte' | 'envelope'; ms: number }[] =
      [
        ...
        { stage: 'sintetizador', lane: 2, kind: 'cte', ms: 2200 },
        { stage: 'analisador', lane: 3, kind: 'cte', ms: 3300 },
        { stage: 'integrador', lane: 4, kind: 'envelope', ms: 4400 },
      ];
