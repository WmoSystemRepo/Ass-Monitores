# Monitor CT-e Carga (POC local · DEV)

Raiz do pacote: **`2-Carga/`**. Esta pasta = app Nx `cte-Carga`.
...
.\serve.cmd
# ou:
npx.cmd nx serve cte-Carga
```

`http://localhost:4260` — Ctrl+F5 após mudanças de UI.
...
| Configurações | Somente leitura · origem Carga | `/config` |
...
- Windows Service original intocado; Ligar usa `tools/Carga.DevHost`
- Debug online só com Ligar pelo Monitor (`monitor-live.log`)
- Paths relativos a `2-Carga/`
