...
/// <summary>
/// Lê monitor-live.log gerado pelo Arquivador.DevHost (Debug.WriteLine do Arquivador).
...
public sealed class LiveTraceReader : ILiveTraceReader
{
    ...
    private string? FindArquivadorRoot()
    {
        ...
        {
            var markerWs = Path.Combine(dir.FullName, "dfend-cte-arquivador-windowsservices");
            var markerApi = Path.Combine(dir.FullName, "Arquivador.Api");
            if (Directory.Exists(markerWs) && Directory.Exists(markerApi))
            {
                return dir.FullName;
            }

            // Também aceita pasta aninhada 2-Arquivador/ como raiz
            var nested = Path.Combine(dir.FullName, "2-Arquivador");
            if (Directory.Exists(Path.Combine(nested, "dfend-cte-arquivador-windowsservices"))
                && Directory.Exists(Path.Combine(nested, "Arquivador.Api")))
            {
                return nested;
            }
    ...
    private static LiveTraceLine? ParseLine(string raw)
    {
        ...
        // Linha crua do Arquivador: DFEND_CTe_Arquivador conexao iniciando...
