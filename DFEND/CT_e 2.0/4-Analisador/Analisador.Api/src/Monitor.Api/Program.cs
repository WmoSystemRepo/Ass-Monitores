...
{
    options.SwaggerDoc("v1", new() { Title = "Monitor.Api (Analisador)", Version = "v1" });
});
...
app.MapGet("/health/ready", async (IMonitorReadRepository repo, CancellationToken ct) =>
{
    ...
    var analiticoOk = await repo.PingAnaliticoAsync(ct);
    if (!primaryOk || !analiticoOk)
    {
        return Results.Json(
            new
            {
                ...
                probe = "readiness",
                primary = primaryOk,
                analitico = analiticoOk
            },
            statusCode: StatusCodes.Status503ServiceUnavailable);
    }

    return Results.Ok(new { status = "ready", probe = "readiness", primary = true, analitico = true });
});
...
// VS / browser na raiz â†’ Swagger (fluxo acostumado)
