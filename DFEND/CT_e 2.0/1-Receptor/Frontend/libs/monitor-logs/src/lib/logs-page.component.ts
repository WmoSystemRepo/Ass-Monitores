...
import { LogEntry } from '@receptor/shared-data';
import {
  LogKind,
  classifyLogKind,
  logKindLabel,
      ...
      <header class="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p class="text-sm text-slate-400">
            Linha do tempo completa do que o Receptor fez no banco.
            @if (store.live()) {
          ...
          @for (l of visibleNewestFirst(); track l.seqLog; let i = $index) {
            ...
            >
              <div class="relative z-10 mt-1 flex w-6 shrink-0 justify-center">
                <span
                  class="h-3 w-3 rounded-full ring-4 ring-slate-950"
                  [ngClass]="dotClass(l)"
                ></span>
              </div>
              <div class="min-w-0 flex-1 rounded-lg border border-slate-800 bg-slate-900/50 px-3 py-2">
                <div class="flex flex-wrap items-center gap-2 text-[11px]">
                  ...
                  @if (l.dtcLog) {
                    <time class="text-slate-400">{{ l.dtcLog | date: 'dd/MM HH:mm:ss' }}</time>
                  }
                  <span class="text-slate-600">#{{ l.seqLog }}</span>
                  @if (l.threadId) {
                    <span class="text-cyan-400/90">Linha {{ l.threadId }}</span>
                  }
                  @if (l.cStat) {
                    <span class="text-violet-300/90">SEFAZ {{ l.cStat }}</span>
                  }
                </div>
                <p class="mt-1 text-sm text-slate-100">{{ summary(l) }}</p>
                @if ((l.mensagem ?? '') !== summary(l)) {
                  <details class="mt-1">
                    <summary class="cursor-pointer text-[11px] text-slate-500 hover:text-slate-300">
                      Ver texto completo
                    </summary>
                    <p class="mt-1 whitespace-pre-wrap font-mono text-[11px] text-slate-400">
                      {{ l.mensagem }}
                    </p>
                  </details>
                }
            ...
            <p class="p-6 text-center text-slate-500">
              Nenhum evento neste filtro. Tente “Todos” ou ligue o Receptor.
            </p>
          }
        </div>
      </div>
