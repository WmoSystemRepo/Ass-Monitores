import { LogEntry, ThreadView } from '@arquivador/shared-data';
...
export interface ThreadStatusInput {
  thread: ThreadView;
  /** Executar do serviço (1 = arquivamento ativo). */
  executar: number;
  /** Processo Arquivador em execução (SCM/DevHost). */
...
export function threadMission(threadId: number): string {
  if (threadId === 1) {
    return 'Worker principal do pool — processa o ciclo de arquivamento.';
  }
...
/** Explica o Δ da posição (só com arquivamento ligado). */
...
/**
 * Status específico da linha — usar só com Arquivador ligado.
