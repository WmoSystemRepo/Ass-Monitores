  ...
  template: `
    <div class="grid shrink-0 grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-5">
      @for (card of items(); track card.key) {
        <a
          [routerLink]="['/monitores', store.serviceId(), 'tabelas', card.key]"
          class="group rounded-md border px-2.5 py-2 transition hover:border-cyan-500/50 hover:bg-slate-900/80"
        ...
        >
          ...
          <p class="mt-1 line-clamp-2 text-xs font-medium text-slate-100">
            {{ card.primaryValue }}
          </p>
          ...
          <p class="mt-1 text-[10px] text-cyan-400/80 opacity-0 transition group-hover:opacity-100">
            Ver dados →
          </p>
        </a>
      }
