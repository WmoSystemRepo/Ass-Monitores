...
import { MonitorApiService, SintetizadorMonitorStore } from '@sintetizador/monitor-core';
import { TableDetailDto } from '@sintetizador/shared-data';
import {
      ...
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        @for (card of cards(); track card.key) {
          <a
            ...
            class="rounded-lg border border-zinc-700 bg-zinc-900/50 p-4 transition hover:border-violet-500/40"
          >
            ...
            <p class="mt-3 text-xs text-violet-400">Ver dados →</p>
          </a>
        } @empty {
...
export class TablesHubPageComponent {
  private readonly store = inject(SintetizadorMonitorStore);
  ...
  template: `
    <section class="flex flex-col gap-4">
      <header class="flex flex-wrap items-end justify-between gap-3">
        <div>
          <a routerLink="/tabelas" class="text-xs text-violet-400 hover:underline">← Tabelas</a>
          <h1 class="mt-1 text-2xl font-semibold text-zinc-50">
            {{ detail()?.label || keyLabel() }}
          </h1>
          <p class="text-sm text-zinc-400">
            Dados da última sessão de síntese
            ...
            <tbody>
              @for (r of detail()?.serviceRows ?? []; track r.nsu) {
                <tr class="border-t border-zinc-800">
                  ...
                  <td class="px-3 py-2 font-mono text-violet-300">{{ r.nsu || '—' }}</td>
        ...
        <div class="max-h-[40vh] overflow-auto rounded border border-zinc-700 bg-zinc-950/60 p-3">
          @for (l of detail()?.contextLogs ?? []; track l.seqLog) {
            <div class="border-b border-zinc-800/80 py-2 text-xs last:border-0">
              ...
              @if (l.cStat) {
                <span class="ml-2 rounded bg-zinc-800 px-1 text-violet-300">cStat {{ l.cStat }}</span>
              }
            ...
            <tbody>
              @for (r of detail()?.configRows ?? []; track r.key) {
                <tr
                  ...
                  [ngClass]="r.key === 'Executar' ? 'bg-violet-950/20' : ''"
                >
                  <td class="px-3 py-2 font-medium text-zinc-200">{{ r.key }}</td>
                  <td class="px-3 py-2 font-mono text-violet-300">{{ r.value }}</td>
      ...
      @if (key() === 'fila') {
        @if (detail()?.fila; as fila) {
          ...
          <div class="rounded border border-zinc-700 bg-zinc-900/40 p-4">
            ...
            <p class="mt-2 font-mono text-sm text-violet-300">
              {{ fila.depthTrend.join(' → ') || '—' }}
            </p>
            <p class="mt-3 text-xs text-zinc-500">
              A fila não lista mensagens individuais — só a quantidade aguardando o Sintetizador.
            </p>
          </div>
        }
      }
