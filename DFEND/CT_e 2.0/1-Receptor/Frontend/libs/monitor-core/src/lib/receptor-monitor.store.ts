...
export class ReceptorMonitorStore {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.bootError.set(
        err instanceof Error ? err.message : 'Falha ao conectar na API DEV'
      );
      this.live.set(false);
    ...
    // Se o hub falhar, REST polling mantém a tela estável.
  ...
  private startRestPolling(): void {
    ...
    setInterval(async () => {
      try {
        ...
        // Só fica offline se o REST também falhar (hub sozinho não pisca).
        this.live.set(false);
      }
    }, 2000);
  }
  ...
  private async connectHub(): Promise<void> {
    ...
    // Não setar live=false no close — causa pisca online/offline; REST cobre o gap.
