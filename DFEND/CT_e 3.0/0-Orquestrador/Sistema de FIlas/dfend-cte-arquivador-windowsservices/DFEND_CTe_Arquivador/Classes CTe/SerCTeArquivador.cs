...
namespace DFe
{
    class SerCTeArquivador
    {
        ...
        public SerCTeArquivador(Facilitador clsFacilPar, string strBDCTeRecepcaoPar, string strBDCTeSinteticoPar, short intCodServicoPar, short intThreadPar)
        {
        ...
        protected void ObterConfigBanco()
        {
            try
            {
                ...
                intReEnviarFila = Convert.ToInt16(clsFacil.ValidarItemConfigBanco(clsBDRec.ObterConfiguracao(intCodServico.ToString(), "ReEnviarFila"), "ReEnviarFila"));
                intQtdeMaxFila = Convert.ToInt16(clsFacil.ValidarItemConfigBanco(clsBDRec.ObterConfiguracao(intCodServico.ToString(), "QtdeMaxFila"), "QtdeMaxFila"));
            }
            catch
            {
                throw;
        ...
        public void Iniciar(ref DateTime datUltimaExecucao)
        {
            ...
            catch (Exception ex)
            {
                try
                {
                    string strErro = "SerCTeArquivador.Iniciar ERRO: " + ex.ToString();
                    System.Diagnostics.Debug.WriteLine(strErro);
                    string strDesktop = System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "DFEND_CTe_Arquivador_erro.txt");
        ...
        public bool Processar()
        {
            ...
            try
            {
                ...
                strChave = clsBDRec.RetirarFilaArquivador();
                ...
                if (strChave != string.Empty)
                {
                    ...
                    // Arquivando documento apartir da chave retirada da fila
                    this.Arquivar(strChave);
                }
            }
            catch (Exception ex)
            {
        ...
        public void Arquivar(string strNSU)
        {
            // Obtendo nome do metodo
            string strMetodo = MethodBase.GetCurrentMethod().Name;
            ...
            try
            {
                ...
                DataSet dstDados = clsBDRec.ObterTempArquivador(strNSU);
                ...
                if ((dstDados != null) && (dstDados.Tables.Count > 0) && (dstDados.Tables[0].Rows.Count > 0))
                {
                    ...
                    // Enviando para a fila do Sintetizador
                    clsNeg.EnviarFilaSintetizador(xmlDocumento, strNSU, strNSUFinal, strProtocolo, strQtde, strEsquema, strData);

                    // Enviando para a fila do Analizador
                    clsNeg.EnviarFilaAnalisador(xmlDocumento, strNSU, strQtde, strEsquema, strData);

                    // Enviando para a fila do Integrador
                    clsNeg.EnviarFilaIntegrador(xmlDocumento, strNSU, strNSUFinal, strQtde, strEsquema, strData);

                    // Excluindo lote arquivado
        ...
        private void ExcluirLote(string strNSU)
        {
            ...
            try
            {
                ...
                clsBDRec.ExcluirTempArquivador(strNSU);
        ...
        public void ReEnviarFila()
        {
            ...
            try
            {
                ...
                if (clsBDRec.ObterQtdeFilaArquivador() < intQtdeMaxFila)
                {
                    // Registrando log de informacao
                    ...
                    DataSet dstDados = clsBDRec.ObterTempArquivadorTop(intQtdeMaxFila);
