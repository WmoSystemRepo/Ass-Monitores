...
/// <summary>
/// Controle operacional do Integrador em POC.
...
public sealed class WindowsServiceControllerAdapter : IWindowsServiceController
{
    ...
    public ServiceControlResult GetStatus(string serviceName)
    {
        ...
        {
            return new ServiceControlResult(true, "Running", "Integrador CT-e ligado.");
        }
        ...
        if (_options.PreferLocalProcess)
        {
            if (TryResolveExePath(out var exePath, out var resolveError))
            {
                return new ServiceControlResult(
                    true,
                    "Stopped",
                    $"Integrador CT-e desligado. Pronto para ligar ({exePath}).");
            }
            ...
            return new ServiceControlResult(
                ...
                "NotFound",
                resolveError
                ?? "Integrador CT-e não disponível. Compile tools/Integrador.DevHost (Debug).");
        }
        ...
        lock (_startLock)
        {
            ...
            if (_options.PreferLocalProcess)
            {
                return new ServiceControlResult(
                    ...
                    "Stopped",
                    killed > 0
                        ? $"Integrador CT-e desligado. Detalhe técnico: host encerrado ({killed}); Executar=0."
                        : "Integrador CT-e desligado. Detalhe técnico: nada em execução; Executar=0.");
            }
        ...
        try
        {
            ...
            if (process.HasExited)
            {
                return new ServiceControlResult(
                    false,
                    "Error",
                    $"Host saiu imediatamente (exit={process.ExitCode}). Verifique {psi.WorkingDirectory}\\Integrador.DevHost.exe.config");
            }

            return new ServiceControlResult(
                true,
                "Running",
                $"Integrador CT-e ligado (PID {process.Id}). Detalhe: Integrador.DevHost — {exePath}");
        }
        catch (Exception ex)
        {
    ...
    private void EnsureDevConfigBesideExe(string exePath)
    {
        ...
        var src = Path.Combine(
            ...
            "AppConfig",
            "Desenvolvimento",
            "DFEND_CTe_Integrador.exe.config");
        if (!File.Exists(src))
        {
            return;
        }
    ...
    private bool TryResolveExePath(out string exePath, out string? error)
    {
        ...
        {
            error =
                "Não achei a pasta 5-Integrador/ (precisa ter dfend-cte-integrador-windowsservices + Integrador.Api). " +
                "Se o Monitor.Api foi iniciado pelo Orquestrador (shadow), reinicie o Orquestrador após atualizar, " +
                "ou defina Monitor:IntegradorRootPath.";
            return false;
        }
        ...
        if (!File.Exists(full))
        {
            var csproj = Path.Combine(root, @"tools\Integrador.DevHost\Integrador.DevHost.csproj");
            if (!DevHostMsBuild.TryBuild(csproj, full, out var buildError))
            {
                error =
                    buildError
                    ?? $"Host POC não encontrado: {full}. Compile tools\\Integrador.DevHost (Debug) ou rode 0-Orquestrador\\tools\\build-devhosts.ps1.";
                return false;
            }
        }
    ...
    private string? FindIntegradorRoot()
    {
                 ...
                 })
        {
            ...
            {
                var hasSvc = Directory.Exists(Path.Combine(dir.FullName, "dfend-cte-integrador-windowsservices"));
                var hasApi = Directory.Exists(Path.Combine(dir.FullName, "Integrador.Api"));
                if (hasSvc && hasApi)
                {
                    return dir.FullName;
                }
    ...
    private bool IsLocalHostRunning()
    {
        try
        {
            var hostName = string.IsNullOrWhiteSpace(_options.IntegradorProcessName)
                ? "Integrador.DevHost"
                : _options.IntegradorProcessName;
