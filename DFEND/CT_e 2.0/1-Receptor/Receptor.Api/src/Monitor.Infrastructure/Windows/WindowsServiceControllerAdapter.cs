...
/// <summary>
/// Controle operacional do Receptor em POC.
...
public sealed class WindowsServiceControllerAdapter : IWindowsServiceController
{
    ...
    public ServiceControlResult GetStatus(string serviceName)
    {
        ...
        {
            return new ServiceControlResult(true, "Running", "Receptor CT-e ligado.");
        }
        ...
        if (_options.PreferLocalProcess)
        {
            if (TryResolveExePath(out var exePath, out var resolveError))
            {
                return new ServiceControlResult(
                    true,
                    "Stopped",
                    $"Receptor CT-e desligado. Pronto para ligar ({exePath}).");
            }
            ...
            return new ServiceControlResult(
                ...
                "NotFound",
                resolveError
                ?? "Receptor CT-e não disponível. Compile tools/Receptor.DevHost (Debug).");
        }
        ...
        lock (_startLock)
        {
            ...
            if (_options.PreferLocalProcess)
            {
                return new ServiceControlResult(
                    ...
                    "Stopped",
                    killed > 0
                        ? $"Receptor CT-e desligado. Detalhe técnico: host encerrado ({killed}); Executar=0."
                        : "Receptor CT-e desligado. Detalhe técnico: nada em execução; Executar=0.");
            }
        ...
        try
        {
            ...
            if (process.HasExited)
            {
                return new ServiceControlResult(
                    false,
                    "Error",
                    $"Host saiu imediatamente (exit={process.ExitCode}). Verifique {psi.WorkingDirectory}\\Receptor.DevHost.exe.config");
            }

            return new ServiceControlResult(
                true,
                "Running",
                $"Receptor CT-e ligado (PID {process.Id}). Detalhe: Receptor.DevHost — {exePath}");
        }
        catch (Exception ex)
        {
    ...
    private bool TryResolveExePath(out string exePath, out string? error)
    {
        ...
        var relative = string.IsNullOrWhiteSpace(_options.ReceptorExeRelativePath)
            ? @"tools\Receptor.DevHost\bin\Debug\Receptor.DevHost.exe"
            : _options.ReceptorExeRelativePath;

        var root = FindReceptorRoot();
        if (root is null)
        {
            error =
                "Não achei a pasta 1-Receptor/ (precisa ter dfend-cte-receptor-windowsservices + Receptor.Api). " +
                "Se o Monitor.Api foi iniciado pelo Orquestrador (shadow), reinicie o Orquestrador após atualizar, " +
                "ou defina Monitor:ReceptorRootPath.";
            return false;
        }
        ...
        if (!File.Exists(full))
        {
            var csproj = Path.Combine(root, @"tools\Receptor.DevHost\Receptor.DevHost.csproj");
            if (!DevHostMsBuild.TryBuild(csproj, full, out var buildError))
            {
                error =
                    buildError
                    ?? $"Host POC não encontrado: {full}. Compile tools\\Receptor.DevHost (Debug) ou rode 0-Orquestrador\\tools\\build-devhosts.ps1.";
                return false;
            }
        }
    ...
    private string? FindReceptorRoot()
    {
        if (!string.IsNullOrWhiteSpace(_options.ReceptorRootPath)
            && Directory.Exists(_options.ReceptorRootPath))
        {
            return Path.GetFullPath(_options.ReceptorRootPath);
        }
                 ...
                 })
        {
            ...
            {
                var hasSvc = Directory.Exists(Path.Combine(dir.FullName, "dfend-cte-receptor-windowsservices"));
                var hasApi = Directory.Exists(Path.Combine(dir.FullName, "Receptor.Api"));
                if (hasSvc && hasApi)
                {
                    return dir.FullName;
                }
    ...
    private bool IsLocalHostRunning()
    {
        try
        {
            var hostName = string.IsNullOrWhiteSpace(_options.ReceptorProcessName)
                ? "Receptor.DevHost"
                : _options.ReceptorProcessName;
