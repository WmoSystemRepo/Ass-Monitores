# Monitor CT-e Arquivador (POC local · DEV)

Raiz do pacote: **`2-Arquivador/`**. Esta pasta = app Nx `cte-arquivador`.
...
Abrir `../Arquivador.Api/Monitor.sln` no VS 2022 · perfil **https** · F5 → Swagger `https://localhost:7126/swagger`.  
Front consome `http://localhost:5020`.
...
.\serve.cmd
# ou:
npx.cmd nx serve cte-arquivador
```

`http://localhost:4210` — Ctrl+F5 após mudanças de UI.
...
| Configurações | Somente leitura · origem Arquivador | `/config` |
...
- Windows Service original intocado; Ligar usa `tools/Arquivador.DevHost`
- Debug online só com Ligar pelo Monitor (`monitor-live.log`)
- Paths relativos a `2-Arquivador/`
