...
public sealed class SqlMonitorRepository : IMonitorReadRepository, IMonitorWriteRepository
{
    ...
    public async Task<QueueReadResult> GetQueueCountsAsync(CancellationToken ct)
    {
        ...
        await using (var cmd = conn.CreateCommand())
        {
            ...
            cmd.CommandText = """
                ...
                FROM cte.tmp_sintetizador_conhecimento_transporte_eletronico WITH (READPAST)
                """;
            await using var reader = await cmd.ExecuteReaderAsync(ct);
            if (await reader.ReadAsync(ct))
            {
        ...
        broker = await TryCountQueueAsync(conn, "fila_alvo_cte_sintetizador", ct);
        swBroker.Stop();

        // Sintetizador: uma fila de entrada; sem fan-out de destino
        ...
        CancellationToken ct)
    {
        try
        {
            ...
            {
                cmd.Parameters.AddWithValue("@cod", _options.CodServicoSintetizador);
            }
