...
import { TableHealthView } from '@sintetizador/shared-data';
import {
    ...
    <div class="grid shrink-0 grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-5">
      @for (card of items(); track card.key) {
        <a
          ...
          class="group rounded-md border px-2.5 py-2 transition hover:border-violet-500/50 hover:bg-zinc-900/80"
        ...
        >
          ...
          <p class="mt-1 text-[10px] text-violet-400/80 opacity-0 transition group-hover:opacity-100">
            Ver dados →
          </p>
        </a>
      }
