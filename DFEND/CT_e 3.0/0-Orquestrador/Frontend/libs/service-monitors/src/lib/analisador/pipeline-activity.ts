 ...
 * para o ciclo do Analisador: fila → temp → classificar → detalhar → limpar.
 */
export function describeLogActivity(log: LogLike): PipelineActivity | null {
  ...
  if (msg.includes('serviço iniciado com sucesso')) {
    ...
    return {
      stage: 'bootstrap',
      headline: 'Analisador ligado',
      detail: machine
        ? `Serviço iniciou na máquina ${machine}`
        : 'Serviço do Analisador acabou de iniciar',
      threadId,
      at: log.dtcLog,
      source,
    };
  ...
  if (msg.includes('processo não iniciado')) {
    return {
      ...
      detail: 'Executar=0 — não analisa até ligar o Analisador',
      threadId,
      at: log.dtcLog,
      source,
    };
  ...
  if (
    ...
    return {
      ...
      detail: nsu
        ? `${threadName} retirou NSU ${nsu} da fila_alvo_cte_analisador`
        : `${threadName} retirou mensagem da fila Analisador`,
      threadId,
      nsu,
      at: log.dtcLog,
      source,
  ...
  if (msg.includes('lote obtido no banco') || msg.includes('dado obtido no banco')) {
    return {
      ...
      detail: nsu
        ? `${threadName} obteve lote NSU ${nsu} em tmp_analise`
        : `${threadName} leu a temporária do analisador`,
      threadId,
      nsu,
      at: log.dtcLog,
      source,
  ...
  // —— Limpar — DELETE temp / AtualizarErro (antes de detalhar genérico) ——
  if (
