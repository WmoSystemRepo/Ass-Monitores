...
namespace DFe
{
    class Threads
    {
        ...
        // Variaveis utilizadas de WebServices
        ...
        private void GravarErro(string strErro)
        {
            ...
            {
                Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "DFEND_CTe_Receptor_erro.txt"),
                Path.Combine(Path.GetTempPath(), "DFEND_CTe_Receptor_erro.txt"),
                Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "DFEND_CTe_Receptor_erro.txt")
            };
            foreach (string strCaminho in arrCaminhos)
            {
                try
                {
                    ...
                    Debug.WriteLine("DFEND_CTe_Receptor erro gravado em: " + strCaminho);
                    break;
                }
                catch
                {
        ...
        public void StartPooledThread()
        {
            ...
            {
                this.GravarErro("DFEND_CTe_Receptor ERRO StartPooledThread: " + ex.ToString());
            }
        }
        ...
        public void Run(short intThread)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    try
                    {
                        ...
                        clsServ = new SerCTeReceptor(clsFacil, strBDCTeRecepcao, strCertificadoDigital, intCodServico, intThread);
                    ...
                    {
                        this.GravarErro("DFEND_CTe_Receptor ERRO Thread " + intThread + ": " + ex.ToString());
                        try
                        {
            ...
            {
                this.GravarErro("DFEND_CTe_Receptor ERRO Run: " + ex.ToString());
            }
            finally
            {
                clsServ = null;
        ...
        #region " ObterConfigCTeReceptor "

        protected void ObterConfigCTeReceptor()
        {
            // Classes e variaveis utilizadas
            ...
            try
            {
                ...
                intCodServico = Convert.ToInt16(appConfig.GetValue("CodServicoReceptor", typeof(string)));

                // Obtendo configuracao do arquivo
                // Desenvolvimento: aceita connection string em texto claro (Data Source=...)
                string strBDConfig = Convert.ToString(clsFacil.ValidarItemConfigArquivo(appConfig.GetValue("BDCTeRecepcao", typeof(string)), "BDCTeRecepcao"));
                if (strBDConfig.StartsWith("Data Source=", StringComparison.OrdinalIgnoreCase))
                {
                    strBDCTeRecepcao = strBDConfig;
                }
                else
                {
                    strBDCTeRecepcao = clsCript.Decriptar(strBDConfig);
                }
                ...
                Debug.WriteLine("DFEND_CTe_Receptor conexao iniciando. Conn=" + strBDCTeRecepcao + " Certificado=" + strCertificadoDigital);
                ...
                this.ObterConfigBancoCTeReceptor(strBDCTeRecepcao);
                Debug.WriteLine("DFEND_CTe_Receptor config banco OK. Threads=" + intThreads + " Intervalo=" + dblIntervalo);
            }
            catch (Exception ex)
            {
                this.GravarErro("DFEND_CTe_Receptor ERRO config/banco: " + ex.ToString());
                throw;
            }
            finally
            {
        ...
        #region " ObterConfigBancoCTeReceptor "

        protected void ObterConfigBancoCTeReceptor(string strBD)
        {
            // Classes e variaveis utilizadas
