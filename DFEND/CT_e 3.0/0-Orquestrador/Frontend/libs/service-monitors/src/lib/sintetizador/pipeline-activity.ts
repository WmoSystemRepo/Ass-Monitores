 ...
 * para o ciclo do Sintetizador: fila → temp → classificar → persistir → limpar.
 */
export function describeLogActivity(log: LogLike): PipelineActivity | null {
  ...
  if (msg.includes('serviço iniciado com sucesso')) {
    ...
    return {
      stage: 'bootstrap',
      headline: 'Sintetizador ligado',
      detail: machine
        ? `Serviço iniciou na máquina ${machine}`
        : 'Serviço do Sintetizador acabou de iniciar',
      threadId,
      at: log.dtcLog,
      source,
    };
  ...
  if (msg.includes('processo não iniciado')) {
    return {
      ...
      detail: 'Executar=0 — não sintetiza até ligar o Sintetizador',
      threadId,
      at: log.dtcLog,
      source,
    };
  ...
  if (
    ...
    return {
      ...
      detail: nsu
        ? `${threadName} retirou NSU ${nsu} da fila_alvo_cte_sintetizador`
        : `${threadName} retirou mensagem da fila Sintetizador`,
      threadId,
      nsu,
      at: log.dtcLog,
      source,
  ...
  if (
    ...
    return {
      ...
      detail: nsu
        ? `${threadName} · NSU ${nsu}`
        : `${threadName} na fila Sintetizador`,
      threadId,
      nsu,
      at: log.dtcLog,
      source,
  ...
  if (msg.includes('lote obtido no banco') || msg.includes('dado obtido no banco')) {
    return {
      ...
      detail: nsu
        ? `${threadName} obteve lote NSU ${nsu} em tmp_sintetizador`
        : `${threadName} leu a temporária do sintetizador`,
      threadId,
      nsu,
      at: log.dtcLog,
      source,
