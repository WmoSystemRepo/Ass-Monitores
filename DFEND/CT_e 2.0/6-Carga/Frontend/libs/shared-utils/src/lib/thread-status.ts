...
export interface ThreadStatusInput {
  ...
  executar: number;
  /** Processo Carga em execução (SCM/DevHost). */
...
export function threadMission(threadId: number): string {
  if (threadId === 1) {
    return 'Worker principal do pool — processa Download pontual · Carga.';
  }
...
/**
 * Chip neutro quando o Carga está parado.
...
/**
 * Status específico da linha — usar só com Carga ligado.
