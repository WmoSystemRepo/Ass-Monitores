...
/** Conexão SignalR do monitor (não confundir com Sintetizador ligado). */
...
export function sintetizadorStatusLabel(scmStatus?: string | null, executar?: number | null): string {
  const status = (scmStatus ?? '').trim();
  const running = status.toLowerCase() === 'running';
  if (running && executar === 1) return 'Ligado';
  if (running && executar !== 1) return 'Síntese pausada';
...
export function summarizeLogMessage(mensagem?: string | null, cStat?: string | null): string {
  ...
  if (cStat === '118') return 'Documentos no ciclo de síntese (cStat 118)';
