# Monitor CT-e Analisador (POC local · DEV)

Raiz do pacote: **`2-Analisador/`**. Esta pasta = app Nx `cte-analisador`.
...
Abrir `../Analisador.Api/Monitor.sln` no VS 2022 · perfil **https** · F5 → Swagger `https://localhost:7136/swagger`.  
Front consome `http://localhost:5040`.
...
.\serve.cmd
# ou:
npx.cmd nx serve cte-analisador
```

`http://localhost:4240` — Ctrl+F5 após mudanças de UI.
...
| Configurações | Somente leitura · origem Analisador | `/config` |
...
- Windows Service original intocado; Ligar usa `tools/Analisador.DevHost`
- Debug online só com Ligar pelo Monitor (`monitor-live.log`)
- Paths relativos a `2-Analisador/`
