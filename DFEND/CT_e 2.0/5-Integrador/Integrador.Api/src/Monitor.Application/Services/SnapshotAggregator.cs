...
public sealed class MonitorOptions
{
    ...
    public string ConnectionStringSintetico { get; set; } = string.Empty;
    public int CodServicoIntegrador { get; set; } = 7;
    public string WindowsServiceName { get; set; } = "DFEND_CTe_Integrador";

    public string ServiceId { get; set; } = "dfend-cte-monitor-integrador";
    public string DisplayName { get; set; } = "Monitor Integrador CT-e";
    public string Domain { get; set; } = "integrador";
    public string ApiVersion { get; set; } = "1.0";
    public string MonitoredService { get; set; } = "DFEND_CTe_Integrador";
    ...
    /// <summary>Opcional. Se vazio, sobe pastas até achar raiz com dfend-cte-integrador-windowsservices + Integrador.Api.</summary>
    public string IntegradorRootPath { get; set; } = string.Empty;
    /// <summary>
    /// Host POC (tools/Integrador.DevHost) — NÃO é o Windows Service original.
    /// Relativo à raiz do clone Integrador (pasta com windowsservices + Integrador.Api).
    /// </summary>
    public string IntegradorExeRelativePath { get; set; } =
        @"tools\Integrador.DevHost\bin\Debug\Integrador.DevHost.exe";
    /// <summary>Nome do processo do host POC (GetProcessesByName).</summary>
    public string IntegradorProcessName { get; set; } = "Integrador.DevHost";
...
public sealed class SnapshotAggregator : ISnapshotAggregator
{
    ...
    public async Task<MonitorSnapshot> BuildSnapshotAsync(CancellationToken ct)
    {
        ...
        try
        {
            var serviceResult = await _read.GetServiceAsync(_options.CodServicoIntegrador, ct);
        ...
        catch (Exception ex)
        {
            ...
            return new MonitorSnapshot(
                ...
                live,
                null,
                TableHealthBuilder.BuildOffline());
        }
    }
        ...
        CancellationToken ct)
    {
        ...
        var result = await _read.GetConfigsAsync(_options.CodServicoIntegrador, ct);
        ...
        IReadOnlyList<LogEntry> logs)
    {
        ...
        for (var id = 1; id <= configured; id++)
        {
            ...
            list.Add(new ThreadView(
                ...
                false,
                last?.Mensagem ?? (id == 1 ? "Consome fila do Integrador" : "Worker do pool"),
                last?.DtcLog,
                last?.CStat,
                last?.SeverityHint));
        }
...
{
    public static IReadOnlyList<TableHealthView> BuildOffline()
    {
        static TableHealthView Card(string key, string label, string route) =>
            new(key, label, "Critico", "Sem conexão SQL", null, 0, "Banco indisponível neste momento.", route);

        return
        [
            Card("servico", "Serviço", "/tabelas/servico"),
            Card("config", "Configuração", "/tabelas/config"),
            Card("tmp", "Temporária", "/tabelas/tmp"),
            Card("log", "Log", "/tabelas/log"),
            Card("fila", "Fila Integrador", "/tabelas/fila"),
            Card("staging", "Staging / Netezza", "/tabelas/staging"),
            Card("fila_fics", "Fila FICS", "/tabelas/fila_fics"),
            Card("fila_doc_vinculado", "Fila DocVinculado", "/tabelas/fila_doc_vinculado")
        ];
    }
        ...
        AlertThresholdOptions thresholds)
    {
        ...
        if (executar == 0)
        {
            ...
            servicoHint = "Integração desligada — serviço ocioso.";
        }
        else if (heartbeatAge.HasValue && heartbeatAge.Value > staleSeconds)
        {
            servicoStatus = "Critico";
            servicoHint = "Última batida no banco antiga com integração ligada.";
        }
        ...
        var servico = new TableHealthView(
            ...
            "Serviço",
            servicoStatus,
            $"Serviço integrador" + (nsuAdvanced ? " · ativo" : string.Empty),
            heartbeatAge,
            servicoQueryMs,
            servicoHint,
            "/tabelas/servico");
        ...
        var configPrimary = executar == 1 ? "Integração ligada" : "Integração desligada";
        ...
        {
            logStatus = "Critico";
            logHint = "Integração ligada sem log novo (silêncio).";
        }
        else if (executar == 0)
        {
            logStatus = "Atencao";
            logHint = "Integração desligada — poucos eventos esperados.";
        }
        ...
        var staging = new TableHealthView(
            "staging",
            "Staging / Netezza",
            "Ok",
            $"Profundidade: {queues.StagingDepth}",
            null,
            0,
            "Semáforo / temps Netezza (quando connection staging configurada).",
            "/tabelas/staging");

        var filaFics = new TableHealthView(
            "fila_fics",
            "Fila FICS",
            "Ok",
            $"Na fila: {queues.FicsDepth}",
            null,
            0,
            "Fila outbound FICS no analítico.",
            "/tabelas/fila_fics");

        var filaDoc = new TableHealthView(
            "fila_doc_vinculado",
            "Fila DocVinculado",
            "Ok",
            $"Na fila: {queues.DocVinculadoDepth}",
            null,
            0,
            "Fila outbound DocVinculado no analítico.",
            "/tabelas/fila_doc_vinculado");

        return [servico, config, tmp, log, fila, staging, filaFics, filaDoc];
    }
}
...
public sealed class TableDetailService : ITableDetailService
{
    ...
    public async Task<TableDetailDto?> GetAsync(string key, int take, CancellationToken ct)
    {
        ...
        var healthKey = key switch
        {
            "configuracao" => "config",
            "temporaria" => "tmp",
            "fila_entrada" or "fila_integrador" => "fila",
            _ => key
        };
        var health = snapshot.TableHealth?.FirstOrDefault(h =>
            h.Key.Equals(healthKey, StringComparison.OrdinalIgnoreCase)
        ...
        var banner = receptionOn
            ? null
            : "Integração desligada — mostrando dados da última sessão (desde a última alteração de Executar).";

        if (snapshot.ConnectionHealth == ConnectionHealth.Down)
        {
            banner = string.IsNullOrWhiteSpace(snapshot.ConnectionError)
                ? "Sem conexão SQL — detalhe indisponível."
                : $"Sem conexão SQL — {snapshot.ConnectionError}";
        }
        ...
        try
        {
            return key switch
            {
                ...
                "fila" or "fila_entrada" or "fila_integrador" => BuildFila(
                    health, sessionStart, receptionOn, banner, snapshot.Queues.ServiceBrokerDepth, snapshot.Queues.BrokerDepthTrend),
                "staging" => BuildDestinationCard(
                    "staging", health, sessionStart, receptionOn, banner,
                    snapshot.Queues.StagingDepth, "Staging / Netezza"),
                "fila_fics" => BuildDestinationCard(
                    "fila_fics", health, sessionStart, receptionOn, banner,
                    snapshot.Queues.FicsDepth, "Fila FICS"),
                "fila_doc_vinculado" => BuildDestinationCard(
                    "fila_doc_vinculado", health, sessionStart, receptionOn, banner,
        ...
        catch (Exception)
        {
            return new TableDetailDto(
                ...
                sessionStart,
                receptionOn,
                banner ?? "Falha ao ler detalhe da tabela.",
                health,
                null,
                null,
                null,
    ...
    private TableDetailDto BuildDestinationCard(
        string key,
        ...
        string label)
    {
        return new TableDetailDto(
            ...
            null,
            null,
            new FilaDetailView(depth, Array.Empty<long>(), _thresholds.FilaAlta,
                depth == 0 ? "Vazia ou sem conexão configurada." : $"Profundidade: {depth}"),
            null);
    }
    ...
    private async Task<TableDetailDto> BuildServicoAsync(
        ...
        CancellationToken ct)
    {
        var service = await _read.GetServiceAsync(_options.CodServicoIntegrador, ct);
        var row = service.Row;
    ...
    private async Task<TableDetailDto> BuildConfigAsync(
        ...
        CancellationToken ct)
    {
        var cfg = await _read.GetProcessConfigsAsync(_options.CodServicoIntegrador, ct);
        return new TableDetailDto(
            "configuracao",
            health.Label,
...
public sealed class ServiceControlService
{
    ...
    public async Task<ServiceControlResult> StartAsync(CancellationToken ct)
    {
        ...
        {
            await _write.SetExecutarAsync(_options.CodServicoIntegrador, 1, ct);
    ...
    {
        var commandId = Guid.NewGuid().ToString("N");
        await _write.SetExecutarAsync(_options.CodServicoIntegrador, 0, ct);
