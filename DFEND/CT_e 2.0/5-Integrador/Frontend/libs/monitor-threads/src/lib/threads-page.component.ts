...
import { LogEntry, ThreadView } from '@integrador/shared-data';
import {
    ...
    >
      <header class="flex shrink-0 flex-wrap items-end justify-between gap-2">
        <div class="min-w-0">
          ...
          >
            {{ receptionRunning() ? 'Em tempo real' : 'Integrador parado' }}
          </p>
          <h1 class="text-xl font-semibold text-zinc-50">Linhas de trabalho</h1>
          <p class="truncate text-xs text-zinc-400">
            @if (receptionRunning()) {
              Pool de workers do Integrador — cada card é uma linha (status, atividade,
              posição).
            } @else {
      ...
      @if (!receptionRunning()) {
        ...
        >
          <p class="min-w-0 truncate">
            O Integrador não está sintetizando. Cards mostram só configuração e última posição.
          </p>
          ...
          @if (card.isHero) {
            ...
            >
              <div class="flex items-start justify-between gap-2">
                ...
                @if (receptionRunning()) {
                  <a
                    class="shrink-0 text-[11px] text-violet-400/90 hover:text-violet-300"
                    [routerLink]="['/logs']"
              ...
              >
                ...
                @if (receptionRunning()) {
                  ...
                  >
                    ...
                    <a
                      class="shrink-0 text-[10px] text-violet-400/90 hover:text-violet-300"
                      [routerLink]="['/logs']"
                      [queryParams]="{ thread: card.thread.threadId }"
                      >Histórico →</a
                    >
