...
public sealed class MonitorProcessLauncher : IMonitorProcessLauncher, IDisposable
{
        ...
        out string? error)
    {
        ...
        try
        {
            ...
            var dllPath = Path.Combine(sourceBinDir, "Monitor.Api.dll");
            ...
            if (File.Exists(dllPath))
            {
                ...
                var shadowDll = Path.Combine(shadowDir, "Monitor.Api.dll");
                _logger.LogDebug(
                    "LocalDev: Monitor.Api {Id} rodando de shadow {Shadow} (origem {Source})",
                    system.Id,
                    shadowDir,
                    sourceBinDir);
            ...
            {
                // Fallback: ainda trava o bin — avisa e exige compile prévio na próxima.
                _logger.LogWarning(
                    "LocalDev: {Dll} ausente — usando dotnet run (pode travar rebuild). Compile Monitor.Api primeiro.",
                    dllPath);
                psi = CreateSilentProcessStartInfo(
                    "dotnet",
