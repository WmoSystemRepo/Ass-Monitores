...
/// <summary>
/// Controle operacional do Sintetizador em POC.
...
public sealed class WindowsServiceControllerAdapter : IWindowsServiceController
{
    ...
    public ServiceControlResult GetStatus(string serviceName)
    {
        ...
        {
            return new ServiceControlResult(true, "Running", "Sintetizador CT-e ligado.");
        }
        ...
        if (_options.PreferLocalProcess)
        {
            if (TryResolveExePath(out var exePath, out var resolveError))
            {
                return new ServiceControlResult(
                    true,
                    "Stopped",
                    $"Sintetizador CT-e desligado. Pronto para ligar ({exePath}).");
            }
            ...
            return new ServiceControlResult(
                ...
                "NotFound",
                resolveError
                ?? "Sintetizador CT-e não disponível. Compile tools/Sintetizador.DevHost (Debug).");
        }
        ...
        lock (_startLock)
        {
            ...
            if (_options.PreferLocalProcess)
            {
                return new ServiceControlResult(
                    ...
                    "Stopped",
                    killed > 0
                        ? $"Sintetizador CT-e desligado. Detalhe técnico: host encerrado ({killed}); Executar=0."
                        : "Sintetizador CT-e desligado. Detalhe técnico: nada em execução; Executar=0.");
            }
        ...
        try
        {
            ...
            if (process.HasExited)
            {
                return new ServiceControlResult(
                    false,
                    "Error",
                    $"Host saiu imediatamente (exit={process.ExitCode}). Verifique {psi.WorkingDirectory}\\Sintetizador.DevHost.exe.config");
            }

            return new ServiceControlResult(
                true,
                "Running",
                $"Sintetizador CT-e ligado (PID {process.Id}). Detalhe: Sintetizador.DevHost — {exePath}");
        }
        catch (Exception ex)
        {
    ...
    private void EnsureDevConfigBesideExe(string exePath)
    {
        ...
        var src = Path.Combine(
            root,
            "dfend-cte-sintetizador-windowsservices",
            "DFEND_CTe_Sintetizador",
            "AppConfig",
            "Desenvolvimento",
            "DFEND_CTe_Sintetizador.exe.config");
        if (!File.Exists(src))
        {
            return;
        }
    ...
    private bool TryResolveExePath(out string exePath, out string? error)
    {
        ...
        var relative = string.IsNullOrWhiteSpace(_options.SintetizadorExeRelativePath)
            ? @"tools\Sintetizador.DevHost\bin\Debug\Sintetizador.DevHost.exe"
            : _options.SintetizadorExeRelativePath;

        var root = FindSintetizadorRoot();
        if (root is null)
        {
            error =
                "Não achei a pasta 3-Sintetizador/ (precisa ter dfend-cte-sintetizador-windowsservices + Sintetizador.Api). " +
                "Se o Monitor.Api foi iniciado pelo Orquestrador (shadow), reinicie o Orquestrador após atualizar, " +
                "ou defina Monitor:SintetizadorRootPath.";
            return false;
        }
        ...
        if (!File.Exists(full))
        {
            var csproj = Path.Combine(root, @"tools\Sintetizador.DevHost\Sintetizador.DevHost.csproj");
            if (!DevHostMsBuild.TryBuild(csproj, full, out var buildError))
            {
                error =
                    buildError
                    ?? $"Host POC não encontrado: {full}. Compile tools\\Sintetizador.DevHost (Debug) ou rode 0-Orquestrador\\tools\\build-devhosts.ps1.";
                return false;
            }
        }
    ...
    private string? FindSintetizadorRoot()
    {
        if (!string.IsNullOrWhiteSpace(_options.SintetizadorRootPath)
            && Directory.Exists(_options.SintetizadorRootPath))
        {
            return Path.GetFullPath(_options.SintetizadorRootPath);
        }
                 ...
                 })
        {
            ...
            {
                var hasSvc = Directory.Exists(Path.Combine(dir.FullName, "dfend-cte-sintetizador-windowsservices"));
                var hasApi = Directory.Exists(Path.Combine(dir.FullName, "Sintetizador.Api"));
                if (hasSvc && hasApi)
                {
                    return dir.FullName;
                }
    ...
    private bool IsLocalHostRunning()
    {
        try
        {
            var hostName = string.IsNullOrWhiteSpace(_options.SintetizadorProcessName)
                ? "Sintetizador.DevHost"
                : _options.SintetizadorProcessName;
