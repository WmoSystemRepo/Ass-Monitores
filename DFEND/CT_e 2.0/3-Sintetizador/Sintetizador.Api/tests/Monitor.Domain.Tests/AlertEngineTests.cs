...
public class AlertEngineTests
{
    ...
    {
        var service = new ServiceStatusView("DFEND_CTe_Sintetizador", "Running", true, 0, "DEV", DateTimeOffset.UtcNow, "Sintetizador");
        ...
        Assert.Contains(alerts, a => a.Code == "SVC_EXECUTAR_OFF"
            && a.Message.Contains("síntese ociosa", StringComparison.OrdinalIgnoreCase));
    }

    [Fact]
    public void Evaluate_BdDown_IsCritical()
    {
        var service = new ServiceStatusView("DFEND_CTe_Sintetizador", "Stopped", false, 0, null, null, null);
        var alerts = AlertEngine.Evaluate(
            service,
            new QueueStats(0, 0, null, Array.Empty<long>()),
            Array.Empty<RecentDocument>(),
    ...
    {
        var service = new ServiceStatusView("DFEND_CTe_Sintetizador", "Running", true, 1, "DEV", DateTimeOffset.UtcNow, "Sintetizador");
        var logs = new[]
        {
