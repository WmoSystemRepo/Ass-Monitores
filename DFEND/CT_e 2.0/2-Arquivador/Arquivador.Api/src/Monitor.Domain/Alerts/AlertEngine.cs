        ...
        DateTimeOffset nowUtc)
    {
        ...
        if (queues.ServiceBrokerDepth >= thresholds.FilaAlta)
        {
            alerts.Add(new MonitorAlert(
                "FILA_ALTA",
                AlertSeverity.Atenção,
                $"Fila Arquivador ≥ {thresholds.FilaAlta} (atual: {queues.ServiceBrokerDepth}).",
                nowUtc));
        }

        if (queues.SintetizadorDepth >= thresholds.FilaAlta
            || queues.AnalisadorDepth >= thresholds.FilaAlta
            || queues.IntegradorDepth >= thresholds.FilaAlta)
        {
            alerts.Add(new MonitorAlert(
                "FILA_DESTINO_ALTA",
                AlertSeverity.Atenção,
                $"Fila destino alta — Sint={queues.SintetizadorDepth}, Anal={queues.AnalisadorDepth}, Int={queues.IntegradorDepth} (limite {thresholds.FilaAlta}).",
                nowUtc));
        }
            ...
            && trend[^1] >= thresholds.FilaCrescendoMinDepth)
        {
            ...
            if (growing)
            {
                alerts.Add(new MonitorAlert(
                    ...
                    $"Fila Arquivador crescendo em {thresholds.FilaCrescendoSnapshots} snapshots (atual: {queues.ServiceBrokerDepth}).",
                    nowUtc));
            }
        }
        ...
        foreach (var log in recentLogs.Take(30))
        {
                ...
                || upper.Contains("MSGDADONAOOBTIDO", StringComparison.Ordinal))
            {
                alerts.Add(new MonitorAlert(
                    "NSU_SEM_TEMP",
                    AlertSeverity.Alerta,
                    "NSU sem temp correspondente (dado não obtido / MsgDadoNaoObtido).",
                    nowUtc));
            }

            if ((upper.Contains("FAN-OUT", StringComparison.Ordinal)
                    || upper.Contains("FANOUT", StringComparison.Ordinal)
                    || upper.Contains("FAN OUT", StringComparison.Ordinal))
                && (upper.Contains("PARCIAL", StringComparison.Ordinal)
                    || upper.Contains("FALHA", StringComparison.Ordinal)))
            {
                alerts.Add(new MonitorAlert(
                    "FANOUT_PARCIAL",
                    AlertSeverity.Alerta,
                    "Fan-out parcial / falha após sintetizador detectada nos logs.",
                    nowUtc));
            }
            else if (upper.Contains("SINTETIZADOR", StringComparison.Ordinal)
                     && upper.Contains("FALHA", StringComparison.Ordinal))
            {
                alerts.Add(new MonitorAlert(
                    "FANOUT_PARCIAL",
                    AlertSeverity.Alerta,
                    "Falha após sintetizador detectada nos logs.",
                    nowUtc));
            }
            ...
            if (upper.Contains("FILA", StringComparison.Ordinal)
                && (upper.Contains("ALTA", StringComparison.Ordinal)
                    || upper.Contains("CHEIA", StringComparison.Ordinal)
                    || upper.Contains("MAX", StringComparison.Ordinal)))
            {
                alerts.Add(new MonitorAlert(
                    "FILA_DESTINO_ALTA",
                    AlertSeverity.Atenção,
                    "Log indica fila alta / cheia.",
                    nowUtc));
            }
        }
