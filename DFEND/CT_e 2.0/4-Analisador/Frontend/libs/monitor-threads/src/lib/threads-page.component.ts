    ...
    >
      <header class="flex shrink-0 flex-wrap items-end justify-between gap-2">
        <div class="min-w-0">
          ...
          >
            {{ receptionRunning() ? 'Em tempo real' : 'Analisador parado' }}
          </p>
          <h1 class="text-xl font-semibold text-zinc-50">Linhas de trabalho</h1>
          <p class="truncate text-xs text-zinc-400">
            @if (receptionRunning()) {
              Pool de workers do Analisador — cada card é uma linha (status, atividade,
              posição).
            } @else {
      ...
      @if (!receptionRunning()) {
        ...
        >
          <p class="min-w-0 truncate">
            O Analisador não está analisando. Cards mostram só configuração e última posição.
          </p>
