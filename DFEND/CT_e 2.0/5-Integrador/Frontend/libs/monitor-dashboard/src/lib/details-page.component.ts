...
import { IntegradorMonitorStore } from '@integrador/monitor-core';
import {
  classifyLogKind,
  describeLogActivity,
  logKindLabel,
    ...
    >
      <header class="flex shrink-0 flex-wrap items-center justify-between gap-2">
        ...
        <div class="flex flex-wrap items-center gap-3">
          <a routerLink="/logs" class="text-xs text-violet-400 hover:underline">Histórico →</a>
          <a routerLink="/" class="text-xs text-violet-400 hover:underline">← Voltar ao Monitor</a>
        </div>
      </header>
      ...
      <div class="grid min-h-0 flex-1 grid-cols-1 gap-2 lg:grid-cols-2">
        ...
        >
          <div class="mb-1.5 shrink-0">
            <h2 class="text-sm font-medium text-zinc-200">Passos do Integrador · online</h2>
          ...
          <div class="min-h-0 flex-1 space-y-1 overflow-hidden">
            @for (row of flowFeed(); track row.key) {
              ...
              <p class="text-xs text-zinc-500">
                Sem passos Debug/SQL. Ligue o Integrador pelo Monitor.
              </p>
            }
          </div>
        </section>
        ...
        >
          <div class="mb-1.5 shrink-0">
            <h2 class="text-sm font-medium text-zinc-200">Últimos eventos SQL do Integrador</h2>
        ...
        >
          <div class="mb-1.5 flex shrink-0 flex-wrap items-center justify-between gap-2">
            <div>
              <h2 class="text-sm font-medium text-zinc-200">Integrador · parâmetros e lotes</h2>
              <p class="text-[10px] text-zinc-500">Configuração ativa e CT-e recentes na temporária</p>
            </div>
            <div class="flex flex-wrap gap-2">
              <a routerLink="/config" class="text-[10px] text-violet-400 hover:underline"
                >Config →</a
              >
            </div>
          </div>
...
export class DetailsPageComponent {
  readonly store = inject(IntegradorMonitorStore);
