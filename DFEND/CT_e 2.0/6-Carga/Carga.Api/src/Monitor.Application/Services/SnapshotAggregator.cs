...
public sealed class MonitorOptions
{
    ...
    public int CodServicoCarga { get; set; } = 99;
    public string WindowsServiceName { get; set; } = "DFEND_CTe_Carga";

    public string ServiceId { get; set; } = "dfend-cte-monitor-carga";
    public string DisplayName { get; set; } = "Monitor Carga CT-e";
    public string Domain { get; set; } = "carga";
    public string ApiVersion { get; set; } = "1.0";
    public string MonitoredService { get; set; } = "DFEND_CTe_Carga";
    ...
    /// <summary>Opcional. Se vazio, sobe pastas até achar raiz com dfend-cte-Carga-windowsservices + Carga.Api.</summary>
    public string CargaRootPath { get; set; } = string.Empty;
    /// <summary>
    /// Host POC (tools/Carga.DevHost) — NÃO é o Windows Service original.
    /// Relativo à raiz do clone Carga (pasta com windowsservices + Carga.Api).
    /// </summary>
    public string CargaExeRelativePath { get; set; } =
        @"tools\Carga.DevHost\bin\Debug\Carga.DevHost.exe";
    /// <summary>Nome do processo do host POC (GetProcessesByName).</summary>
    public string CargaProcessName { get; set; } = "Carga.DevHost";
...
public sealed class SnapshotAggregator : ISnapshotAggregator
{
    ...
    public async Task<MonitorSnapshot> BuildSnapshotAsync(CancellationToken ct)
    {
        ...
        try
        {
            var serviceResult = await _read.GetServiceAsync(_options.CodServicoCarga, ct);
        ...
        CancellationToken ct)
    {
        ...
        var result = await _read.GetConfigsAsync(_options.CodServicoCarga, ct);
        ...
        IReadOnlyList<LogEntry> logs)
    {
        ...
        for (var id = 1; id <= configured; id++)
        {
            ...
            list.Add(new ThreadView(
                ...
                false,
                last?.Mensagem ?? (id == 1 ? "Consome fila do Carga" : "Worker do pool"),
                last?.DtcLog,
                last?.CStat,
                last?.SeverityHint));
        }
...
public static class TableHealthBuilder
{
        ...
        AlertThresholdOptions thresholds)
    {
        ...
        if (executar == 0)
        {
            ...
            servicoHint = "Carga desligada — serviço ocioso.";
        }
        else if (heartbeatAge.HasValue && heartbeatAge.Value > staleSeconds)
        {
            servicoStatus = "Critico";
            servicoHint = "Última batida no banco antiga com Carga ligada.";
        }
        ...
        var servico = new TableHealthView(
            ...
            "Serviço",
            servicoStatus,
            $"Serviço Carga" + (nsuAdvanced ? " · ativo" : string.Empty),
            heartbeatAge,
            servicoQueryMs,
            servicoHint,
            "/tabelas/servico");
        ...
        var configPrimary = executar == 1 ? "Carga ligada" : "Carga desligada";
        ...
        {
            logStatus = "Critico";
            logHint = "Carga ligada sem log novo (silêncio).";
        }
        else if (executar == 0)
        {
            logStatus = "Atencao";
            logHint = "Carga desligada — poucos eventos esperados.";
        }
...
public sealed class TableDetailService : ITableDetailService
{
    ...
    public async Task<TableDetailDto?> GetAsync(string key, int take, CancellationToken ct)
    {
        ...
        var banner = receptionOn
            ? null
            : "Carga desligada — mostrando dados da última sessão (desde a última alteração de Executar).";
    ...
    private async Task<TableDetailDto> BuildServicoAsync(
        ...
        CancellationToken ct)
    {
        var service = await _read.GetServiceAsync(_options.CodServicoCarga, ct);
        var row = service.Row;
    ...
    private async Task<TableDetailDto> BuildConfigAsync(
        ...
        CancellationToken ct)
    {
        var cfg = await _read.GetProcessConfigsAsync(_options.CodServicoCarga, ct);
        return new TableDetailDto(
            "configuracao",
            health.Label,
...
public sealed class ServiceControlService
{
    ...
    public async Task<ServiceControlResult> StartAsync(CancellationToken ct)
    {
        ...
        {
            await _write.SetExecutarAsync(_options.CodServicoCarga, 1, ct);
    ...
    {
        var commandId = Guid.NewGuid().ToString("N");
        await _write.SetExecutarAsync(_options.CodServicoCarga, 0, ct);
