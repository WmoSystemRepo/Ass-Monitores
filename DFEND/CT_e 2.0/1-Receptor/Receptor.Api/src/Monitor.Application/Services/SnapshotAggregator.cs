...
public sealed class MonitorOptions
{
    ...
    public int CodServicoReceptor { get; set; } = 2;
    public string WindowsServiceName { get; set; } = "DFEND_CTe_Receptor";

    public string ServiceId { get; set; } = "dfend-cte-monitor-receptor";
    public string DisplayName { get; set; } = "Monitor Receptor CT-e";
    public string Domain { get; set; } = "receptor";
    public string ApiVersion { get; set; } = "1.0";
    public string MonitoredService { get; set; } = "DFEND_CTe_Receptor";
    ...
    /// <summary>Opcional. Se vazio, sobe pastas até achar Receptor/ (dfend-cte-receptor-windowsservices + Receptor.Api).</summary>
    public string ReceptorRootPath { get; set; } = string.Empty;
    /// <summary>
    /// Host POC (tools/Receptor.DevHost) — NÃO é o Windows Service original.
    /// Relativo à raiz Receptor/. Path antes de Receptor/ pode variar entre máquinas.
    /// </summary>
    public string ReceptorExeRelativePath { get; set; } =
        @"tools\Receptor.DevHost\bin\Debug\Receptor.DevHost.exe";
    /// <summary>Nome do processo do host POC (GetProcessesByName).</summary>
    public string ReceptorProcessName { get; set; } = "Receptor.DevHost";
...
public sealed class SnapshotAggregator : ISnapshotAggregator
{
    ...
    public async Task<MonitorSnapshot> BuildSnapshotAsync(CancellationToken ct)
    {
        ...
        try
        {
            var serviceResult = await _read.GetServiceAsync(_options.CodServicoReceptor, ct);
        ...
        CancellationToken ct)
    {
        ...
        var result = await _read.GetConfigsAsync(_options.CodServicoReceptor, ct);
    ...
    {
        string Cfg(string key) => configs.TryGetValue(key, out var v) ? v : "0";
            ...
            string? fallbackHint = null)
        {
            ...
            return new ThreadView(
                ...
                isIdle,
                outsideDatabase,
                last?.Mensagem ?? fallbackHint,
                last?.DtcLog,
                last?.CStat,
                last?.SeverityHint);
        }

        var nsuAux = Cfg("NSUAux");
        var nsuAut = Cfg("NSUAuxAut");
        var nsuDest = Cfg("NSUAuxDest");

        return
        [
            Make(1, "Principal", "num_sequencial_unico", 2, mainNsu, false, false),
            Make(2, "Auxiliar", "NSUAux", 2, nsuAux, nsuAux == "0", false),
            Make(3, "Autorização", "NSUAuxAut", 0, nsuAut, nsuAut == "0", false),
            Make(4, "Destinatário", "NSUAuxDest", 1, nsuDest, nsuDest == "0", false),
            Make(5, "Arquivo", "NSU.txt", 2, null, false, true, "NSU fora do BD (arquivo local)")
        ];
    }
}
...
public static class TableHealthBuilder
{
        ...
        AlertThresholdOptions thresholds)
    {
        ...
        if (executar == 0)
        {
            ...
            servicoHint = "Recepção desligada — NSU não deve avançar.";
        }
        else if (heartbeatAge.HasValue && heartbeatAge.Value > staleSeconds)
        {
            servicoStatus = "Critico";
            servicoHint = "Última batida no banco antiga com recepção ligada.";
        }
        ...
        var configPrimary = executar == 1 ? "Recepção ligada" : "Recepção desligada";
        ...
        {
            logStatus = "Critico";
            logHint = "Recepção ligada sem log novo (silêncio).";
        }
        else if (executar == 0)
        {
            logStatus = "Atencao";
            logHint = "Recepção desligada — poucos eventos esperados.";
        }
        ...
        // --- Fila ---
        var filaStatus = "Ok";
        var filaHint = "Fila do Arquivador estável.";
        if (queues.BrokerDepth >= thresholds.FilaAlta)
        {
            filaStatus = "Critico";
            filaHint = "Fila alta — Arquivador atrasado.";
        }
        ...
        {
            filaStatus = "Atencao";
            filaHint = "Fila crescendo nos últimos ciclos.";
        }
...
public sealed class TableDetailService : ITableDetailService
{
    ...
    public async Task<TableDetailDto?> GetAsync(string key, int take, CancellationToken ct)
    {
        ...
        return key switch
        {
            ...
            "fila" => BuildFila(health, sessionStart, receptionOn, banner, snapshot),
            _ => null
        };
    }
    ...
    private async Task<TableDetailDto> BuildServicoAsync(
        ...
        CancellationToken ct)
    {
        var service = await _read.GetServiceAsync(_options.CodServicoReceptor, ct);
        var row = service.Row;
    ...
    private async Task<TableDetailDto> BuildConfigAsync(
        ...
        CancellationToken ct)
    {
        var cfg = await _read.GetProcessConfigsAsync(_options.CodServicoReceptor, ct);
        return new TableDetailDto(
            "configuracao",
            health.Label,
    ...
    {
        var depth = snapshot.Queues.ServiceBrokerDepth;
        var trend = snapshot.Queues.BrokerDepthTrend;
...
public sealed class ServiceControlService
{
    ...
    public async Task<ServiceControlResult> StartAsync(CancellationToken ct)
    {
        ...
        {
            await _write.SetExecutarAsync(_options.CodServicoReceptor, 1, ct);
    ...
    {
        var commandId = Guid.NewGuid().ToString("N");
        await _write.SetExecutarAsync(_options.CodServicoReceptor, 0, ct);
