...
{
    options.SwaggerDoc("v1", new() { Title = "Monitor.Api (Receptor)", Version = "v1" });
});
...
{
    var ok = await repo.PingAsync(ct);
    if (!ok)
    {
        return Results.Json(
            new { status = "unhealthy", probe = "readiness", primary = false },
            statusCode: StatusCodes.Status503ServiceUnavailable);
    }

    return Results.Ok(new { status = "ready", probe = "readiness", primary = true });
});
