...
/// <summary>
/// Lê monitor-live.log gerado pelo Receptor.DevHost (Debug.WriteLine do Receptor).
...
public sealed class LiveTraceReader : ILiveTraceReader
{
    ...
    private string? FindReceptorRoot()
    {
        ...
        {
            var markerWs = Path.Combine(dir.FullName, "dfend-cte-receptor-windowsservices");
            var markerApi = Path.Combine(dir.FullName, "Receptor.Api");
            if (Directory.Exists(markerWs) && Directory.Exists(markerApi))
            {
                return dir.FullName;
            }

            // Também aceita pasta Receptor/ como raiz
            var nested = Path.Combine(dir.FullName, "Receptor");
            if (Directory.Exists(Path.Combine(nested, "dfend-cte-receptor-windowsservices"))
                && Directory.Exists(Path.Combine(nested, "Receptor.Api")))
            {
                return nested;
            }
    ...
    private static LiveTraceLine? ParseLine(string raw)
    {
        ...
        // Linha crua do Receptor: DFEND_CTe_Receptor conexao iniciando...
