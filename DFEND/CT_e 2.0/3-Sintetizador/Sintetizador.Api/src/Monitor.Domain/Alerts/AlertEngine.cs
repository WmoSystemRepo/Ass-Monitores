        ...
        DateTimeOffset nowUtc)
    {
        ...
        if (queues.ServiceBrokerDepth >= thresholds.FilaAlta)
        {
            alerts.Add(new MonitorAlert(
                "FILA_ALTA",
                AlertSeverity.Atenção,
                $"Fila Sintetizador ≥ {thresholds.FilaAlta} (atual: {queues.ServiceBrokerDepth}).",
                nowUtc));
        }
            ...
            && trend[^1] >= thresholds.FilaCrescendoMinDepth)
        {
            ...
            if (growing)
            {
                alerts.Add(new MonitorAlert(
                    ...
                    $"Fila Sintetizador crescendo em {thresholds.FilaCrescendoSnapshots} snapshots (atual: {queues.ServiceBrokerDepth}).",
                    nowUtc));
            }
        }
        ...
        if (window.Any(d => d.HasError))
        {
            alerts.Add(new MonitorAlert(
                "TMP_ERRO",
                AlertSeverity.Alerta,
                "Há des_mensagem_erro nos lotes recentes de tmp_sintetizador.",
                nowUtc));
            alerts.Add(new MonitorAlert(
                "TEMP_COM_ERRO",
                AlertSeverity.Alerta,
