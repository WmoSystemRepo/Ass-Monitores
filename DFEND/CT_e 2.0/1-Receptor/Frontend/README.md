# Monitor CT-e Receptor (POC local · DEV)

Raiz do pacote: **`1-Receptor/`**. Esta pasta = app Nx `cte-receptor`.
...
Abrir `../Receptor.Api/Monitor.sln` no VS 2022 · perfil **https** · F5 → Swagger `https://localhost:7116/swagger`.  
Front consome `http://localhost:5010`.
...
.\serve.cmd
# ou:
npx.cmd nx serve cte-receptor
```

`http://localhost:4200` — Ctrl+F5 após mudanças de UI.
...
```powershell
cd C:\Users\wmoliveira\Desktop\Clones\CT_e\Receptor\Frontend
...
| Configurações | Somente leitura · origem Receptor | `/config` |

Rota auxiliar (sem item de menu — link no fluxo do Monitor): `/mais-informacoes`.

## Endpoints (BFF)

- `GET /api/monitor/snapshot` — inclui `liveTrace`
- `GET /api/monitor/logs?afterSeq=&take=`
- `GET /api/monitor/service/status`
- `POST /api/monitor/service/start` — DevHost + `Executar=1` (`PreferLocalProcess`)
- `POST /api/monitor/service/stop` — encerra host + `Executar=0`
- Hub `/hubs/monitor` — `snapshot`, `logsAppend`

## Observações

- Dados reais SQL **DEV** — sem mock
- Windows Service original intocado; Ligar usa `tools/Receptor.DevHost`
- Debug online só com Ligar pelo Monitor (`monitor-live.log`)
- Paths relativos a `1-Receptor/`
