...
public sealed class SqlMonitorRepository : IMonitorReadRepository, IMonitorWriteRepository
{
    ...
    public async Task<QueueReadResult> GetQueueCountsAsync(CancellationToken ct)
    {
        ...
        broker = await TryCountQueueAsync(conn, "fila_alvo_cte_arquivador", ct);
        swBroker.Stop();

        long sint = 0, anal = 0, integ = 0;
        if (!string.IsNullOrWhiteSpace(_options.ConnectionStringSintetico))
        {
            try
            {
                await using var connSint = await OpenAsync(ct, _options.ConnectionStringSintetico);
                sint = await TryCountQueueAsync(connSint, "fila_alvo_cte_sintetizador", ct);
                anal = await TryCountQueueAsync(connSint, "fila_alvo_cte_analisador", ct);
                integ = await TryCountQueueAsync(connSint, "fila_alvo_cte_integrador", ct);
            }
            catch
            {
                // depths remain 0
        ...
        {
            sint = await TryCountQueueAsync(conn, "fila_alvo_cte_sintetizador", ct);
            anal = await TryCountQueueAsync(conn, "fila_alvo_cte_analisador", ct);
            integ = await TryCountQueueAsync(conn, "fila_alvo_cte_integrador", ct);
        }
        ...
        CancellationToken ct)
    {
        try
        {
            ...
            {
                cmd.Parameters.AddWithValue("@cod", _options.CodServicoArquivador);
            }
    ...
    private static string ClassifyLogSeverity(string? msg, string? cStat)
    {
        ...
        if (msg.Contains("lote já existente", StringComparison.OrdinalIgnoreCase)
            ...
            || msg.Contains("arquivado", StringComparison.OrdinalIgnoreCase)
