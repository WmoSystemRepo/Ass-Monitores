...
public class AlertEngineTests
{
    ...
    {
        var service = new ServiceStatusView("DFEND_CTe_Arquivador", "Running", true, 0, "DEV", DateTimeOffset.UtcNow, "Arquivador");
        ...
        Assert.Contains(alerts, a => a.Code == "SVC_EXECUTAR_OFF"
            && a.Message.Contains("arquivamento ocioso", StringComparison.OrdinalIgnoreCase));
    }

    [Fact]
    public void Evaluate_BdDown_IsCritical()
    {
        var service = new ServiceStatusView("DFEND_CTe_Arquivador", "Stopped", false, 0, null, null, null);
        var alerts = AlertEngine.Evaluate(
            service,
            new QueueStats(0, 0, null, Array.Empty<long>()),
            Array.Empty<RecentDocument>(),
    ...
    {
        var service = new ServiceStatusView("DFEND_CTe_Arquivador", "Running", true, 1, "DEV", DateTimeOffset.UtcNow, "Arquivador");
        var logs = new[]
        {
