...
@Component({
  ...
  standalone: true,
  imports: [DatePipe, RouterLink, IntegradorAnatomyFlowComponent, TableHealthCardsComponent],
    ...
    <section class="dashboard-fit flex h-[calc(100vh-3rem)] max-h-[calc(100vh-3rem)] flex-col gap-2 overflow-hidden">
        ...
        <div class="min-w-0 flex flex-wrap items-center gap-2.5">
          <div class="min-w-0">
            ...
            <p class="text-[11px] text-zinc-400">
              Está integrando documentos agora?
            </p>
          </div>
      ...
      @if (isLimitedTelemetry()) {
        ...
        >
          ...
          @if (processUp()) {
            <span class="font-medium"> Processo no ar; integração sem telemetria completa.</span>
          }
          <button
            type="button"
        ...
        >
          <span>
            @if (isLimitedTelemetry()) {
              ...
              Integração pausada (Executar≠1). Ative para voltar a integrar CT-e.
            }
          </span>
          <button
            type="button"
          ...
          <div class="receptor-empty flex h-full flex-col items-center justify-center gap-4 rounded-xl border border-zinc-600/60 bg-zinc-950/40 px-6 text-center">
            <div class="max-w-md space-y-2">
              <p class="text-[10px] font-semibold uppercase tracking-[0.16em] text-violet-300/90">
                Pipeline do Integrador
              </p>
              <h2 class="text-lg font-semibold text-zinc-50">Integrador parado</h2>
              <p class="text-sm text-zinc-400">
                Ligue para integrar CT-e da fila e acompanhar o fluxo:
...
export class IntegradorDashboardPageComponent {
  ...
  /** Status único legível em 2s — substitui Processo/Integração fragmentados. */
  readonly heroStatus = computed(() => {
    ...
    if (this.isRunning()) {
      ...
      return {
        ...
        hint: 'Integrador ligado — aguardando próximo ciclo de integração',
      };
    }
    if (this.processUp()) {
      return {
    ...
    return {
      ...
      label: 'Parado',
      detail: null as string | null,
      hint: 'Integrador desligado — não processa novos CT-e',
    };
  });
  ...
  readonly flowCaption = computed(() => {
    ...
    if (!this.isRunning()) {
      ...
      if (this.processUp()) {
        return 'Processo no ar, mas a integração está pausada (Executar≠1). Ative o trabalho para ver o pipeline.';
      }
