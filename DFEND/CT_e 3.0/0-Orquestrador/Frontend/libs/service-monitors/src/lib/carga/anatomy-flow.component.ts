...
/** Estágios dDownload pontual · Carga (diagrama fluxo — 5 plataformas). */
...
@Component({
  selector: 'lib-carga-anatomy-flow',
  standalone: true,
        ...
        <div class="flex flex-wrap items-center gap-2">
          @if (latest(); as lote) {
            <div class="rounded-lg border border-zinc-600/70 bg-zinc-900/70 px-3 py-1.5 shadow-sm">
              <p class="text-[9px] font-semibold uppercase tracking-wide text-teal-400">Último lote</p>
          ...
          <a
            ...
            class="rounded border border-zinc-600 bg-zinc-900/60 px-2.5 py-1 text-[11px] font-medium text-teal-300 hover:bg-zinc-800"
          >
            Mais informações →
          </a>
        </div>
        ...
        <div class="anatomy-fill-board relative flex h-full min-w-[880px] flex-col items-stretch px-2 py-2">
          <div class="anatomy-cycle-block relative z-[1] mx-auto w-full">
            ...
            <div class="anatomy-summary-bar mt-3 shrink-0">
              ...
              <div title="Threads">
                <a
                  routerLink="/monitores/carga/threads"
                  class="anatomy-summary-label hover:text-teal-300"
...
export class CargaAnatomyFlowComponent implements OnDestroy {
  ...
  count(id: AnatomyStage): string {
    ...
    switch (id) {
      ...
      case 'persistir': {
        ...
        return q != null && q > 0 ? `${q} doc(s)` : 'sintético';
      }
      case 'limpar':
        return 'DELETE / órfão';
    }
