...
/// <summary>
/// Lê monitor-live.log gerado pelo Carga.DevHost (Debug.WriteLine do Carga).
...
public sealed class LiveTraceReader : ILiveTraceReader
{
    ...
    private string? FindCargaRoot()
    {
        ...
        {
            var markerWs = Path.Combine(dir.FullName, "dfend-cte-carga-windowsservices");
            var markerApi = Path.Combine(dir.FullName, "Carga.Api");
            if (Directory.Exists(markerWs) && Directory.Exists(markerApi))
            {
                return dir.FullName;
            }

            // Também aceita pasta aninhada 2-Carga/ como raiz
            var nested = Path.Combine(dir.FullName, "2-Carga");
            if (Directory.Exists(Path.Combine(nested, "dfend-cte-carga-windowsservices"))
                && Directory.Exists(Path.Combine(nested, "Carga.Api")))
            {
                return nested;
            }
    ...
    private static LiveTraceLine? ParseLine(string raw)
    {
        ...
        // Linha crua do Carga: DFEND_CTe_Carga conexao iniciando...
