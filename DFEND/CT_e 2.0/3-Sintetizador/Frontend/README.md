# Monitor CT-e Sintetizador (POC local · DEV)

Raiz do pacote: **`2-Sintetizador/`**. Esta pasta = app Nx `cte-sintetizador`.
...
Abrir `../Sintetizador.Api/Monitor.sln` no VS 2022 · perfil **https** · F5 → Swagger `https://localhost:7136/swagger`.  
Front consome `http://localhost:5030`.
...
.\serve.cmd
# ou:
npx.cmd nx serve cte-sintetizador
```

`http://localhost:4230` — Ctrl+F5 após mudanças de UI.
...
| Configurações | Somente leitura · origem Sintetizador | `/config` |
...
- Windows Service original intocado; Ligar usa `tools/Sintetizador.DevHost`
- Debug online só com Ligar pelo Monitor (`monitor-live.log`)
- Paths relativos a `2-Sintetizador/`
