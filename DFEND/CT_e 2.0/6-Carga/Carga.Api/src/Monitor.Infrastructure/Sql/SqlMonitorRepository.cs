...
public sealed class SqlMonitorRepository : IMonitorReadRepository, IMonitorWriteRepository
{
    ...
    private SqlConnection CreateConnection(string? connectionString = null)
    {
        ...
        if (string.IsNullOrWhiteSpace(cs))
        {
            throw new InvalidOperationException(
                "ConnectionString DEV nÃ£o configurada. Use User Secrets ou appsettings.Development.json.");
        }
    ...
    public async Task<bool> PingSinteticoAsync(CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(_options.ConnectionStringSintetico))
        {
            return true;
        }
        ...
        try
        {
            await using var conn = await OpenAsync(ct, _options.ConnectionStringSintetico);
            await using var cmd = conn.CreateCommand();
            cmd.CommandTimeout = _options.SqlTimeoutSeconds;
            cmd.CommandText = "SELECT 1";
            _ = await cmd.ExecuteScalarAsync(ct);
    ...
    {
        // Temp + fila Integrador vivem no sintético; config/serviço/log na recepção.
        var sintCs = string.IsNullOrWhiteSpace(_options.ConnectionStringSintetico)
            ? null
            : _options.ConnectionStringSintetico;
        ...
        try
        {
            await using var conn = await OpenAsync(ct, sintCs);
            await using (var cmd = conn.CreateCommand())
            {
                cmd.CommandTimeout = _options.SqlTimeoutSeconds;
                cmd.CommandText = """
            ...
            swTemp.Stop();
            broker = await TryCountQueueAsync(conn, "fila_alvo_cte_integrador", ct);
            swBroker.Stop();
        }
        catch
        {
        ...
        CancellationToken ct)
    {
        ...
        await using var cmd = conn.CreateCommand();
        cmd.CommandTimeout = _options.SqlTimeoutSeconds;
        // Temp Integrador: des_esquema = chave; sem des_mensagem_erro formal (órfãos = temp que ficou).
        cmd.CommandText = """
            SELECT TOP (@take)
              num_sequencial_unico,
              num_sequencial_unico_final,
              qtd_documento,
              CAST(NULL AS varchar(max)) AS des_mensagem_erro,
        ...
        CancellationToken ct)
    {
        try
        {
            ...
            {
                cmd.Parameters.AddWithValue("@cod", _options.CodServicoCarga);
            }
    ...
    {
        if (cStat is "118" or "117" or "129" or "130" or "131" or "146" or "730" or "992")
        {
            return "success";
        }
        ...
        if (msg.Contains("lote jÃ¡ existente", StringComparison.OrdinalIgnoreCase)
            ...
            || ((msg.Contains("fan-out", StringComparison.OrdinalIgnoreCase)
                    || msg.Contains("fanout", StringComparison.OrdinalIgnoreCase))
                && !msg.Contains("falha", StringComparison.OrdinalIgnoreCase)
                && !msg.Contains("parcial", StringComparison.OrdinalIgnoreCase)))
        {
            return "success";
        }
